...
def _average(*denoisers: Any) -> Any:
    ...
    pair and returns their arithmetic mean. It is therefore directly
    compatible with DeepInverse's plug-and-play prior and with
    :func:`pulserver.recon.pics`.

    Denoisers may be passed positionally or as one sequence.
    """
    ...
    class _AverageDenoiser(models.Denoiser):
...
def _llr(
    *,
    ...
    """Create a locally low-rank blockwise singular-value denoiser.

    The model treats the channel axis as the low-rank contrast/subspace axis
    and soft-thresholds singular values independently in overlapping spatial
    ...
    class _LLRDenoiser(models.Denoiser):
        def __init__(self) -> None:
            super().__init__()
        ...
        def _coordinates(
            ...
            for size, block, step in zip(
                ...
                starts.append(
                    torch.tensor(axis_starts, dtype=torch.long, device=device)
                )
        ...
        def forward(self, x: Any, sigma: Any, **kwargs: Any) -> Any:
            ...
            if threshold.ndim == 0:
                ...
                raise ValueError(
                    "sigma must be a scalar or contain one value per batch"
                )
            ...
            if weights is not None:
                output = (
                    output / weights.clamp_min(torch.finfo(real_dtype).eps)[None, None]
                )
