"""Grouping and filtering helpers for MRD acquisition streams.
...
def filter_acquisitions(
    ...
    *,
    ...
    for acquisition in acquisitions:
        if all(has_acquisition_flag(acquisition, flag) for flag in required) and not any(
            has_acquisition_flag(acquisition, flag) for flag in rejected
        ):
            yield acquisition
