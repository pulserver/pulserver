"""EPI acquisition-role helpers; phase fitting stays with an upstream backend.
...
def partition_epi_acquisitions(
    ...
    for acquisition in acquisitions:
        if _has_any_flag(acquisition, ("ACQ_IS_PHASECORR_DATA", "ACQ_IS_NAVIGATION_DATA")):
            ...
            acquisition, ("ACQ_IS_PARALLEL_CALIBRATION", "ACQ_IS_PARALLEL_CALIBRATION_AND_IMAGING")
        ):
            single_band_reference.append(acquisition)
        else:
            imaging.append(acquisition)
    return EpiAcquisitionGroups(phase_correction, single_band_reference, reverse_polarity, imaging)
