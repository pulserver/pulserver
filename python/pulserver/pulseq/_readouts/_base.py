...
def compute_readout_duration(
    ...
    a_ticks = int(round(a / time_unit))
    g_ticks = int(round(g / time_unit))
    if a_ticks <= 0 or g_ticks <= 0:
        raise ValueError("time_unit too large relative to raster times; decrease time_unit.")
    ...
    if T_ticks % g_ticks != 0:
        raise RuntimeError("Computed T_readout is not a multiple of grad_raster_time (check time_unit).")
    if (q * a_ticks) % a_ticks != 0:
        raise RuntimeError("Computed dwell is not a multiple of adc_raster_time (unexpected).")
