...
def make_spsp_pulse(
    ...
    spat_cancel_alpha_phs : bool, default=False
        For 'ex' pulses, absorb the alpha phase
        profile from beta's spatial profile, so they cancel for a flatter
        total phase.
    freq_cancel_alpha_phs : bool, default=False
        For 'ex' pulses, absorb the alpha phase
    ...
    frequency_profile = _dzbeta(
        ...
        cancel_alpha_phs=spat_cancel_alpha_phs,
    )
    frequency_profile = _padded_centered_fft(frequency_profile, 2 * n_samples, n_samples)
    ...
    frequency_weighting = _dzbeta(
        ...
        cancel_alpha_phs=freq_cancel_alpha_phs,
    )
    
    # Generate weighted RF frequency profile
    weighted_profile = np.outer(frequency_profile, frequency_weighting) * np.sin(flip_angle / 2.0)
    ...
    raw_signal = np.stack([_padded_centered_fft(np.sin(0.5 * prof), 2 * n_samples, n_samples) for prof in hybrid_prof], axis=-1)
    ...
    if flyback:
        ...
        g_rew_pad = _round_duration(g_rew_dur, system.rf_raster_time, system.grad_raster_time)
...
def _round_duration(T_s: float, r1_s: float, r2_s: float) -> float:
    ...
    r1_i = int(round(r1_s * scale))
    r2_i = int(round(r2_s * scale))
...
def _versec(g: NDArray[float], rf: NDArray[complex]) ->NDArray[complex]:
    ...
    interp_vals = np.vstack([
        np.interp(k, x, rf[:, j]) for j in range(n)
    ]).T
