...
class Sequence(pp.Sequence):
    ...
    def write(
        ...
        raise NotImplementedError("pulserver.pulseq.Sequence is a fast builder and does not implement write().")

    def read(self, *_args, **_kwargs) -> None:
        """Fast builder does not support reading/parsing external sequence files."""
        raise NotImplementedError("pulserver.pulseq.Sequence is a fast builder and does not implement read().")
    ...
    def set_block(self, _block_index: int, *args: SimpleNamespace | float) -> None:  # noqa: ARG002
        ...
        raise NotImplementedError(
            "pulserver.pulseq.Sequence only supports sequential add_block(). "
    ...
    def get_block(self, block_index: int) -> SimpleNamespace:
        ...
        del block_index
        raise NotImplementedError("pulserver.pulseq.Sequence is a fast builder and does not implement get_block().")
    ...
    def _fast_set_block(self, block_index: int, *args: SimpleNamespace | float) -> None:
        ...
        for event in events:
            ...
            if event.type == "rf":
                ...
                raise ValueError(f"Unknown event type {event.type} passed to pulserver.pulseq.Sequence.add_block().")
