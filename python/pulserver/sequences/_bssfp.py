...
def bSSFP3D(fov, npix, flip_angle_deg, TR, osf=2.0, rbw=250.0, phase_inc_deg=180.0, system=None, params=None):
    if params is None:
        params = {}
    ...
    gx = pp.make_trapezoid(
        channel='x',
        flat_area=Nx * deltakx,
        flat_time=readout_dur,
        system=system
    )
    ...
    # min TR
    minTE = 0.5 * pp.calc_duration(rf) + pp.calc_duration(gx_pre) + 0.5 * pp.calc_duration(gx, adc)
    minTR = 2 * minTE
    if TR < minTR:
        logging.warning(f'Minimum TR {minTR*1e3:.2f} ms is shorter than requested {TR*1e3:.2f} ms')
        TR = minTR
    ...
    prep_delay_time = max(0.0, TR / 2.0 - pp.calc_duration(gz) if hasattr(pp, 'calc_duration') else max(0.0, TR / 2.0 - gz_dur))
    ...
    for i in range(Ny):
        ...
        if i == 0:
            gy_pre_prev = pp.make_trapezoid(channel='y', area=phase_areas[0], system=system)
        else:
            gy_pre_prev = pp.make_trapezoid(channel='y', area=phase_areas[i - 1], system=system)

        # current PE step
        gy = pp.make_trapezoid(channel='y', area=phase_areas[i], system=system, duration=gx_dur)
