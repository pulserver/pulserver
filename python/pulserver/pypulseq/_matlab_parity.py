...
import numpy as np
from pypulseq.supported_labels_rf_use import get_supported_rf_uses as _get_supported_rf_uses
...
def calc_rf_power(rf, dt: float = 1e-6) -> tuple[float, float, float]:
    ...
    samples = np.interp(time, np.asarray(rf.t, dtype=float), np.asarray(rf.signal), left=0.0, right=0.0)
...
def make_hexagon_gradient_area(
    ...
    def solve(steps):
        return _hexagon_solution(steps, area, grad_start, grad_end, max_slew, max_grad, raster)
...
def _hexagon_solution(duration, area, grad_start, grad_end, max_slew, max_grad, raster):
    ...
    reachable = up_min * (amplitude + grad_start) + down_min * (amplitude + grad_end) + 2 * flat * amplitude

    if abs(2 * area / raster) > abs(reachable):
        return None

    flat_topped = _flat_topped_solution(duration, area, grad_start, grad_end, max_slew, max_grad, raster, sign)
    ...
    while abs(2 * area / raster) < abs(reachable):
        ...
        flat = max(duration - up_min - down_min, 0.0)
        reachable = (
            up_min * (amplitude + grad_start) + down_min * (amplitude + grad_end) + 2 * flat * amplitude
        )
    ...
    while flat > max(flat_floor, -1):
        ...
        up = up_min + step
        down = down_min + step
        flat = duration - min(down_min + step, down_limit) - min(up_min + step, up_limit)

        if flat <= 0:
            triangles = _triangle_candidates(duration, area, grad_start, grad_end, max_slew, raster, sign)
            break
        ...
        if abs(corner_up) >= max_grad:
            corner_up, up, up_limit = _clamp_corner(
                sign, max_slew, max_grad, raster, grad_start, corner_down, up_limit, flat
            )
        ...
        current = (raster / 2) * (
            up * (corner_up + grad_start) + flat * (corner_up + corner_down) + down * (corner_down + grad_end)
        )
    ...
    return _triangle_solution(triangles, duration, area, grad_start, grad_end, max_slew, max_grad, raster)
...
def _clamp_corner(sign, max_slew, max_grad, raster, edge, other_corner, limit, flat):
    ...
    on_the_limit = abs(((limit + flat) * sign * max_grad + edge - other_corner) / (limit + flat))
...
def _corner_solution(area, grad_start, grad_end, corner_up, corner_down, up, down, flat, raster, max_slew):
    ...
    if abs(corner_up) < abs(corner_down):
        fixed = corner_down
        free = -(grad_start * up + grad_end * down + fixed * (flat + down) - 2 * area / raster) / (flat + up)
        amplitudes = np.array([grad_start, free, fixed, grad_end])
    else:
        fixed = corner_up
        free = -(grad_start * up + grad_end * down + fixed * (flat + up) - 2 * area / raster) / (flat + down)
        amplitudes = np.array([grad_start, fixed, free, grad_end])

    if np.max(np.abs(np.diff(amplitudes) / np.diff(times))) <= max_slew + _LIMIT_TOLERANCE:
        return times, amplitudes
    return None


def _flat_topped_solution(duration, area, grad_start, grad_end, max_slew, max_grad, raster, sign):
    """The plateau shape, tried first because it is the one that usually works."""
    up = (duration * max_slew * raster + sign * (grad_end - grad_start)) / (2 * max_slew * raster)
    ...
    return _plateau(duration, area, grad_start, grad_end, max_slew, max_grad, raster, up_steps, down_steps)


def _flat_topped_scan(duration, area, grad_start, grad_end, max_slew, max_grad, raster, up, down):
    ...
    for up_steps in range(max(up - 1, 0), duration - down + 1):
        for down_steps in range(max(down - 1, 0), duration - up_steps + 1):
            found = _plateau(
                duration, area, grad_start, grad_end, max_slew, max_grad, raster, up_steps, down_steps
            )
            if found is not None:
                return found
    return None
...
def _plateau(duration, area, grad_start, grad_end, max_slew, max_grad, raster, up_steps, down_steps):
...
def _triangle_candidates(duration, area, grad_start, grad_end, max_slew, raster, sign):
    ...
    low = numerator / (grad_start - grad_end + sign * duration * max_slew * raster) / raster
    high = duration - numerator / (-grad_start + grad_end + sign * duration * max_slew * raster) / raster
    return [
        (up, down)
...
def _triangle_solution(candidates, duration, area, grad_start, grad_end, max_slew, max_grad, raster):
    ...
    for up, down in candidates:
        ...
        if flat < 0:
            continue
        amplitude = -(up * raster * grad_start + down * raster * grad_end - 2 * area) / (
