...
def _labels(value: int) -> tuple:
    return (
        make_label(label="NOPOS", type="SET", value=value),
        make_label(label="NOROT", type="SET", value=value),
    )
...
class FatSaturationPulse(RfModule):
    def __init__(self, system, rf, spoiler: tuple, labels_on: tuple, labels_off: tuple, duration: float):
        ...
        super().__init__(system, ((rf, *labels_on), (*spoiler, *labels_off)))
...
class DiffusionPrepPulse(RfModule):
    def __init__(
        ...
        blocks: list[tuple] = [(self.rf90_down, *self.labels_on), (self.gradient,)]
        ...
        blocks.extend(((self.gradient,), (self.rf90_up,), (*self.spoiler, *self.labels_off)))
