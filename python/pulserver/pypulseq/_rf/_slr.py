...
def _dinf(d1: float, d2: float) -> float:
    ...
    l10d1 = np.log10(d1)
    l10d2 = np.log10(d2)
    return float((a1 * l10d1 * l10d1 + a2 * l10d1 + a3) * l10d2 + (a4 * l10d1 * l10d1 + a5 * l10d1 + a6))
...
def _least_squares(n: int, tbw: float, d1: float, d2: float) -> np.ndarray:
    ...
    bands = np.asarray([0.0, (1.0 - transition) * tbw / 2.0, (1.0 + transition) * tbw / 2.0, n / 2.0]) / (n / 2.0)
    ...
    phase = np.exp(1j * 2.0 * np.pi / (2.0 * (n + 1)) * np.concatenate((np.arange(0, n / 2 + 1), np.arange(-n / 2, 0))))
    return np.real(np.fft.ifft(np.fft.fft(h) * phase))[:n]


def _linear_phase(n: int, tbw: float, d1: float, d2: float) -> np.ndarray:
    transition = _dinf(d1, d2) / tbw
    bands = np.asarray([0.0, (1.0 - transition) * tbw / 2.0, (1.0 + transition) * tbw / 2.0, n / 2.0]) / n
...
def _minimum_phase(n: int, tbw: float, d1: float, d2: float) -> np.ndarray:
    ...
    bands = np.asarray([0.0, (1.0 - transition) * tbw / 2.0, (1.0 + transition) * tbw / 2.0, n / 2.0]) / n
    linear = signal.remez(n2, bands, [1.0, 0.0], weight=[1.0, 2.0 * d1 / (0.5 * d2 * d2)], fs=1.0)
...
def _beta_to_rf(beta: np.ndarray, cancel_alpha_phase: bool) -> np.ndarray:
    ...
    if cancel_alpha_phase:
        beta_spectrum = np.fft.fft(beta) * np.exp(-1j * np.angle(np.fft.fft(alpha[::-1])))
