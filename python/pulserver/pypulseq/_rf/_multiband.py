...
def make_multiband(
    ...
    *,
    ...
    Convert any user-designed RF module and append the resulting blocks::
        ...
        base = pp.make_frequency_selective_pulse(np.deg2rad(30), 300.0)
        pulse = pp.make_multiband(base, 4, 1000.0, sideband_power=2.0)
    ...
    .. plot::
       ...
       base = lambda offset: pp.make_frequency_selective_pulse(
           np.deg2rad(30), 300.0, freq_offset=offset, system=system)
       frequency_profile_gallery([
           ("two bands left", pp.make_multiband(base(-2000.0), 2, 1000.0)),
           ("two bands right", pp.make_multiband(base(1000.0), 2, 1000.0)),
           ("three bands (MATLAB reference)", pp.make_multiband(base(0.0), 3, 1000.0)),
       ])
    """
...
def make_multiband_frequency_selective_pulse(
    ...
    **kwargs
    ...
    Four equispaced saturation bands, always including 0 Hz::
        ...
        pulse = pp.make_multiband_frequency_selective_pulse(
    ...
    .. plot::
       ...
       system = pp.Opts(max_grad=40, grad_unit="mT/m", max_slew=150, slew_unit="T/m/s")
       rf_figure(pp.make_multiband_frequency_selective_pulse(
...
def make_pins(
    ...
    *,
    ...
    Transform a user-designed slice-selective module and append it::
        ...
        base = pp.make_slice_selective_pulse(np.deg2rad(15), 3e-3)
        pulse = pp.make_pins(base, 6, 18e-3, slice_thickness=3e-3)
        seq = pp.Sequence(pulse.system)
        for block in pulse:
            seq.add_block(*block)
    ...
    .. plot::
       ...
       from _figures import rf_figure
       system = pp.Opts(max_grad=40, grad_unit="mT/m", max_slew=150, slew_unit="T/m/s")
       base = pp.make_slice_selective_pulse(np.deg2rad(15), 3e-3, system=system)
       rf_figure(pp.make_pins(base, 6, 18e-3, slice_thickness=3e-3),
...
def make_pins_slice_selective_pulse(
    ...
    **kwargs
    ...
    Design six slices, offset from the nominal slice by default::
        ...
        pulse = pp.make_pins_slice_selective_pulse(
    ...
    .. plot::
       ...
       from _figures import rf_figure
       system = pp.Opts(max_grad=40, grad_unit="mT/m", max_slew=150, slew_unit="T/m/s")
       rf_figure(pp.make_pins_slice_selective_pulse(
