...
class TRSequence(pp.Sequence):
    ...
    def __init__(
        ...
        *,
        ...
        self.block_durations = {
            number: block["duration"] for number, block in enumerate(self._blocks, start=1)
        }
        self.block_events = {
    ...
    def rf_over(self, block_number: int) -> list[tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    @classmethod
    def from_c(cls, waveforms: dict, system: pp.Opts, *, rf_channel: int = 0) -> TRSequence:
        ...
        adc_events = [
            {
                ...
                "dwell": float(event["duration_us"]) * 1e-6 / max(int(event["num_samples"]), 1),
...
def _grad_over(channel: np.ndarray, start: float, duration: float) -> SimpleNamespace | None:
...
def overlay_rf_channels(waveform: TRSequence, magnitude_axes, phase_axes, time_factor) -> None:
