...
Unlike a balanced :class:`~pulserver.pypulseq._readout.line.Line2D` shot,
...
class _Bssfp(Readout):
    ...
    def set_state(self, segment_idx: int = 0, rf_phase_rad: float = 0.0):
        segment_idx = int(segment_idx)
        if not 0 <= segment_idx < self.num_segments:
            raise IndexError(f"segment_idx must be in [0, {self.num_segments}), got {segment_idx}")
        self._replace_state(_BssfpState(segment_idx, float(rf_phase_rad)))
        return self
    ...
    def _build_blocks(self):
        ...
        rf_half.phase_offset = state.rf_phase_rad
        yield tuple(
        ...
        for encoded_idx in range(start, stop):
            ...
            phase = state.rf_phase_rad + np.pi * (encoded_idx & 1)
