...
class Zte(Readout):
    ...
    def set_state(self, lin_idx=0, rf_phase_rad=0.0, rotation=None):
        ...
        phases = _schedule(rf_phase_rad, self.num_views, "rf_phase_rad", integer=False)
        self._replace_state(
            _ZteState(
                lin_idx=labels,
                rf_phase_rad=phases,
    ...
    def _build_blocks(self):
        ...
        for index, gradients in enumerate(self._view_gradients):
            ...
            rf.phase_offset += state.rf_phase_rad[index]
    ...
    def __call__(self, seq=None, lin_idx=0, rf_phase_rad=0.0, rotation=None):
        """Append one complete ordered segment, or return it standalone."""
        self.set_state(lin_idx=lin_idx, rf_phase_rad=rf_phase_rad, rotation=rotation)
