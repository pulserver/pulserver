...
Three callable readout objects, built once and then called per shot to append a
...
loop live in the caller. The object exposes the timing the caller needs
(:attr:`esp`, :attr:`duration`, :attr:`t_first_echo`,
:attr:`t_exc_center_to_train_start`, :attr:`refoc_center_s`) to align its 90 and
pad TR.
...
and the gradient never returns to zero needlessly — exactly the construction of
the pypulseq TSE demo (after Kelley/Zaitsev's ``writeTSE``). The repeating unit,
built from component spoiler/readout trapezoids, is

    z:  [GS4 flat slice-select] [GS5 ref→0]           0        [GS7 0→ref]
    x:            0             [GR5 0→ro ] [GR6 readout+ADC] [GR7 ro→0 ]
    y:            0             [phase-enc]      0            [ unwind  ]
        --------- 180 ---------  ----------- readout -----------

``GS5``/``GS7`` fuse the z crusher into the slice-select ramp; ``GR5``/``GR7``
fuse the x spoiler into the readout ramp. A one-off lead-in prephaser (x) with a
0→ref z crusher precedes the first 180; a ref→0 lobe closes out.
...
from ..encoding import CRUSHER_CYCLES_Z
from ..system import DEFAULT_BANDWIDTH_HZ_PX, apply_system_derates, ceil_to_raster, quantize_readout_timing, round_to_raster

RF_REFOCUS_TIME_S = 3.0e-3
RF_REFOCUS_APODIZATION = 0.5
RF_REFOCUS_TIME_BW_PRODUCT = 4.0
CPMG_PHASE_OFFSET_RAD = math.pi / 2  # Hennig CPMG condition: 90 deg from the excitation phase
...
# TRAPS variable-refocusing ramp (Alsop 1997 "opt" schedule) coefficients.
_TRAPS_OPT_EX1 = 2.0
_TRAPS_OPT_CA = 0.4
_TRAPS_OPT_CB = 2.0
_TRAPS_OPT_CC = 2.0
...
def build_z_crusher(opts: pp.Opts, slice_thickness_m: float, cycles: float = CRUSHER_CYCLES_Z):
    """Standalone slice-axis crusher trapezoid (helper; the trains fuse their own)."""
...
def build_refocusing_pulse(
    ...
    *,
    ...
    """Build a reference ``(rf, gz)`` slice/slab-selective refocusing pulse.

    Used as the readout objects' default when the caller does not supply one.
    Only ``gz.amplitude`` is used (the slice-select is replayed as a fused
    flat-only lobe); per-echo flip variation is an envelope scaling.
    """
    rf, gz, _ = pp.make_sinc_pulse(
        flip_angle=math.radians(flip_angle_deg),
        duration=duration_s,
...
class _FseTrain:
    ...
    def __init__(
        ...
        *,
        ...
        if refoc_rf is not None and refoc_gz is None:
            raise ValueError("a slice/slab-selective refocusing pulse needs its gradient (pass refoc_gz)")
        ...
        self.slice_thickness_m = float(slice_thickness_m)
        ...
        # --- refocusing pulse + fused flat slice-select (GS4) -------------
        if refoc_rf is None:
            refoc_rf, refoc_gz = build_refocusing_pulse(system, self.slice_thickness_m)
        self._rf = refoc_rf
        ...
        self._t_refwd = system.rf_dead_time + RF_REFOCUS_TIME_S + system.rf_ringdown_time
        ...
        dk, kw = self._ro["delta_k"], self._ro["k_width"]
        ...
        ro_amp, rt = self._ro["ro_amp"], self._ro["readout_time_s"]
        self._Sx = x_spoil_factor * kw                                   # x crusher area
        self._dx_reb = 0.5 * ro_amp * (rt - 2.0 * self._ro["adc_echo_offset_s"])
        self._prephase_area = self._Sx + 0.5 * ro_amp * rt
        ...
        # --- ESP / tau timing ---------------------------------------------
        c180 = self.refoc_center_s
        adc_echo = self._ro["adc_echo_offset_s"]
        half_pre_fixed = (self._t_refwd - c180) + adc_echo
        half_post_fixed = c180 + (self._ro["readout_time_s"] - adc_echo)
        ...
        self._gr5 = _fuse_lobe(system, ro_axis, self._Sx + self._dx_reb - 0.5 * self._dG * self._ro["ro_amp"],
                               0.0, self._ro["ro_amp"], self._t_sp_pre, self._dG)
        self._gr7 = _fuse_lobe(system, ro_axis, self._Sx - self._dx_reb - 0.5 * self._dG * self._ro["ro_amp"],
                               self._ro["ro_amp"], 0.0, self._t_sp_post, self._dG)
        ...
        self._t_lead = ceil_to_raster(
            max(self._t_sp_pre, _lobe_dur(system, self._dG, self._prephase_area),
                _lobe_dur(system, self._dG, self._Cz)), raster)
        self._gr_pre = pp.make_trapezoid(channel=ro_axis, area=self._prephase_area, duration=self._t_lead, rise_time=self._dG, system=system)
        self._gs_up = _fuse_lobe(system, "z", self._Cz, 0.0, self._ref_amp, self._t_lead, self._dG)
        self._gs_down = _fuse_lobe(system, "z", self._Cz, self._ref_amp, 0.0, self._t_sp_post, self._dG)

        # --- exposed timing + exact duration ------------------------------
        self.t_first_echo = self._t_lead + c180 + half_esp
        self.t_exc_center_to_train_start = half_esp - (self._t_lead + c180)
    ...
    def _make_axis(self, spec):
        if spec is None:
            return None
        axis, fov_m, n, label = spec
        delta_k = 1.0 / float(fov_m)
        areas = (np.arange(n) - 0.5 * n) * delta_k
        return {"axis": axis, "n": int(n), "label": label, "areas": areas,
                "max_area": float(np.max(np.abs(areas))) if n > 0 else 0.0}
    ...
    def _scaled_refoc(self, echo_idx, freq_offset):
        ...
        rf.phase_offset = self._phase[echo_idx]
    ...
    def _run(self, seq, lin_idx, par_idx, freq_offset):
        if seq is None:
            import pulserver.pulseq as _ps
        ...
        for e in range(self.etl):
            seq.add_block(self._gs4, self._scaled_refoc(e, freq_offset))

            gs5, gs7 = self._gs5, self._gs7
            labels, pre_grads, rew_grads = [], [], []
            if self._pe is not None:
                ky = int(lin[e])
                area = self._pe["areas"][ky]
                pre_grads.append(pp.make_trapezoid(channel=self._pe["axis"], area=area, duration=self._t_sp_pre, rise_time=self._dG, system=self._opts))
                rew_grads.append(pp.make_trapezoid(channel=self._pe["axis"], area=-area, duration=self._t_sp_post, rise_time=self._dG, system=self._opts))
            ...
            if self._multi_echo and e > 0:
                labels.append(pp.make_label(type="INC", label="ECO", value=1))

            adc = copy.deepcopy(self._ro["adc"])
            adc.phase_offset = self._phase[e]
            seq.add_block(self._gr5, gs5, *pre_grads)
            seq.add_block(self._ro["gr6"], adc, *labels)
            seq.add_block(self._gr7, gs7, *rew_grads)
...
class MultiEchoSE(_FseTrain):
    ...
    increments from the second echo on, labelling echoes ``0..etl-1``.
    """

    def __init__(self, system, fov_x_m, nx, etl, slice_thickness_m, **kw):
        super().__init__(system, fov_x_m, nx, etl, slice_thickness_m, multi_echo=True, **kw)

    def __call__(self, seq=None, freq_offset=0.0):
        """Append one shot's multi-echo train. ``freq_offset`` (Hz) selects the
        slice via the refocusing pulse. Returns the sequence."""
        return self._run(seq, None, None, freq_offset)
...
class Fse2D(_FseTrain):
    ...
    refoc_rf, refoc_gz : optional
        Swappable refocusing pulse + its slice-select gradient (default sinc).
    ...
    def __init__(self, system, fov, matrix, etl, slice_thickness_m, *, pe_axis="y", **kw):
        super().__init__(system, fov[0], matrix[0], etl, slice_thickness_m,
                         pe_spec=(pe_axis, fov[1], matrix[1], "LIN"), **kw)
        self.pe_axis = pe_axis
        self.fov_y_m, self.ny = float(fov[1]), int(matrix[1])

    def __call__(self, seq=None, lin_idx=0, freq_offset=0.0):
        """Append one shot. ``lin_idx`` is a scalar (same ky every echo) or a
        length-``etl`` array (one ky per echo — the FSE case); ``freq_offset``
        (Hz) selects the slice. Returns the sequence."""
        return self._run(seq, lin_idx, None, freq_offset)
...
class Fse3D(_FseTrain):
    ...
    ``slab_thickness_m`` sizes the slab-selective refocusing pulse and the z
    crusher; the kz partition encode is folded into the z crusher lobes with
    :func:`pypulseq.add_gradients` (``GS5 += kz``, ``GS7 -= kz``) so the net
    z-moment per unit stays zero. A single slab is excited, so there is no
    per-slice frequency offset. See :class:`Fse2D` for the shared parameters.
    """

    def __init__(self, system, fov, matrix, etl, slab_thickness_m, *, pe_axis="y", par_axis="z", **kw):
        super().__init__(system, fov[0], matrix[0], etl, slab_thickness_m,
    ...
    def __call__(self, seq=None, lin_idx=0, par_idx=0):
        """Append one shot. ``lin_idx``/``par_idx`` are scalars or length-``etl``
        arrays giving the per-echo ky/kz line and partition indices. Returns the
        sequence."""
        return self._run(seq, lin_idx, par_idx, 0.0)
