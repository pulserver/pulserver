...
def build_z_crusher(opts: pp.Opts, thickness_m: float, cycles: float = CRUSHER_CYCLES_Z):
    ...
    return pp.make_trapezoid(channel="z", area=cycles / thickness_m, system=opts)
...
class MultiEchoSE(_FseTrain):
    ...
    def __call__(self, seq=None, freq_offset=0.0):
        ...
        slice via the refocusing pulse's frequency offset (RF phase is
        compensated accordingly). Returns the sequence."""
...
class Fse2D(_FseTrain):
    ...
    def __call__(self, seq=None, lin_idx=0, freq_offset=0.0):
        ...
        (Hz) selects the slice via the refocusing pulse's frequency offset (RF
        phase is compensated accordingly). Returns the sequence."""
