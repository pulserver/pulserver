...
class _FseReadout(SequenceModule):
    ...
    def init_module(
        ...
        *,
        ...
        gx_bridge_pre = bridge(system, "x", ramp_area + spoil_area + rebalance, 0.0, amplitude)
        ...
        lead_span = max(lead_floor, pp.round_to_raster(0.5 * esp_first - lead_fixed, raster))
        pre_span = pp.round_to_raster(0.5 * esp - pre_fixed, raster)
        pre_span = min(max(pre_span, pre_floor), esp - ref_span - flat_span - post_floor)
        ...
        first_echo = exc_span + lead_span + ref_span + wait_first + pre_span + echo_offset
...
def _encodes(system: pp.Opts, resolutions, sign: float, duration: float | None) -> tuple:
    ...
    encodes = [
        pp.scale_grad(
            pp.make_phase_encoding(axis, resolution, system=system, duration=duration), sign
        )
