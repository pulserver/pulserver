...
refocusing pulse -- see :mod:`pulserver.design.readout.fse` for
...
Labels use ``LIN``/``PAR`` ``labelinc`` (never ``labelset``) for every line
after the first -- the caller ``labelset``s ``LIN``/``PAR`` to the train's
starting index before calling it; the train only ever increments from there,
by exactly the jump the sampling mask dictates.

Rotation
--------
``__call__`` accepts an optional ``rotation`` (a scipy-``Rotation``-like
object, see :func:`pulserver.pulseq.make_rotation`), attached to every block
...
def _parse_mask(sampling_mask, has_par):
    ...
    if mask.ndim != 1:
        raise ValueError("sampling_mask must be 1D -- (ky_idx,) per line -- when no partition axis is configured")
    return mask, None
...
class _EpiTrainBlipped:
    ...
    def _run(self, seq, pe_start, rotation):
        if seq is None:
        ...
        for j in range(self.etl):
            ...
            if j > 0:
                if self._pe is not None:
                    labels.append(pp.make_label(type="INC", label=self._pe.label, value=int(self._mask_pe[j] - self._mask_pe[j - 1])))
                if self._par is not None:
                    labels.append(pp.make_label(type="INC", label=self._par.label, value=int(self._mask_par[j] - self._mask_par[j - 1])))
    ...
    def _run(self, seq, pe_start, rotation):
        ...
        for j in range(self.etl):
            ...
            if j < self.etl - 1:
                ...
                if self._pe is not None:
                    gap_events.append(pp.make_label(type="INC", label=self._pe.label, value=int(self._mask_pe[j + 1] - self._mask_pe[j])))
                if self._par is not None:
                    gap_events.append(pp.make_label(type="INC", label=self._par.label, value=int(self._mask_par[j + 1] - self._mask_par[j])))
                if rotation is not None:
...
class Epi2D(_EpiTrainBlipped):
    ...
    def __call__(self, seq=None, pe_start=0, rotation=None):
        ...
        object (see :func:`pulserver.pulseq.make_rotation`) applied to every
        block of the shot. Returns the sequence."""
        return self._run(seq, pe_start, rotation)
...
class Epi3D(_EpiTrainBlipped):
    ...
    def __call__(self, seq=None, pe_start=(0, 0), rotation=None):
