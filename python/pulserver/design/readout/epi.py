...
from ._common import as_tuple, left_align_rephaser, present, solve_delay, solve_rephasing
    ...
    def init_module(
        ...
        *,
        ...
        if flyback:
            ...
            gx = [pp.scale_grad(lobe, 1.0 if line % 2 == 0 else -1.0) for line in range(etl)]
            esp = pp.calc_duration(lobe)
            gy_blips = _blip_events(system, "y", steps[:, 0], delta_k[1], blip_span, lobe, etl)
            gz_blips = (
        ...
        gz_pre = pp.make_phase_encoding("z", fov[2] / n[2], system=system) if ndim == 3 else None
        pre_span = pp.ceil_to_raster(
            max(pp.calc_duration(event) for event in (gx_pre, gy_pre, *present(gz_pre))), raster
        )
        gx_pre = pp.make_trapezoid(
            channel="x", area=-0.5 * _area(lobe), duration=pre_span, system=system
        )
        gy_pre = pp.make_phase_encoding("y", fov[1] / n[1], system=system, duration=pre_span)
        if ndim == 3:
            gz_pre = pp.make_phase_encoding("z", fov[2] / n[2], system=system, duration=pre_span)
        ...
        gz_reph = left_align_rephaser(gz_reph, ("x", "y", "z"[: ndim - 2]), type(self).__name__)
        shot_labels = tuple(
...
def _read_lobe(
    system: pp.Opts, k_width: float, sample_span: float, blip_span: float, ramp_sampling: bool
):
...
def _blip_events(
    ...
    events = []
    for line in range(etl):
        leaving = halves[line][0] if line < etl - 1 and halves[line] is not None else None
        entering = halves[line - 1][1] if line and halves[line - 1] is not None else None
        events.append(_flank(system, entering, leaving, anchor))
    return events


def _gap_blips(
    system: pp.Opts, channel: str, steps: np.ndarray, delta_k: float, gap_span: float, etl: int
) -> list:
    ...
    events = [
        pp.scale_grad(template, float(step) / widest) if step else None for step in steps
    ]
    return [*events, None]
