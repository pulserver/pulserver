...
:mod:`pulserver.design.readout.epi` / :mod:`pulserver.design.readout.fse`.
...
-- it never advances the phase encode within a shot, so (see below) it never
touches the ``LIN``/``PAR`` labels itself.
...
``INC ECO 1`` on every following one. ``LIN``/``PAR`` are never touched here
-- the caller ``labelset``s them (once per shot, since ``ky``/``kz`` is fixed
for the whole shot) alongside whatever RF/slice block precedes the call.
...
:mod:`pulserver.design.readout.fse`'s ``x_spoil_factor``), not from an
...
``(0.5, 1]``) -- exactly :mod:`pulserver.design.readout.fse`'s ``pf``.
...
the train emits -- same convention as :mod:`pulserver.design.readout.epi`.
"""
...
def _build_readout_x(opts, ro_axis, nx, fov_x_m, bandwidth_hz_px, oversamp, pf):
    ...
    semantics to :func:`pulserver.design.readout.fse._build_readout`.
    """
    if not (0.5 < pf <= 1.0):
...
class _LineTrain:
    ...
    def __init__(
        ...
        *,
        ...
        if self._flyback:
            last_positive = self._start_pos
        else:
            last_positive = ((self.num_echoes - 1) % 2 == 0) == self._start_pos
...
class Line2D(_LineTrain):
    ...
    def __call__(self, seq=None, pe_idx=0, rf_phase_rad=0.0, rotation=None):
        ...
        caller ``labelset``s ``LIN`` to this value (once) before calling.
        ``rf_phase_rad`` is set as every echo ADC's ``phase_offset`` (RF
        spoiling). ``rotation`` is an optional scipy ``Rotation``-like object
        (see :func:`pulserver.pulseq.make_rotation`) applied to every block.
        Returns the sequence."""
        return self._run(seq, pe_idx, 0, rf_phase_rad, rotation)
...
class Line3D(_LineTrain):
    ...
    def __call__(self, seq=None, pe_idx=0, par_idx=0, rf_phase_rad=0.0, rotation=None):
        ...
        ``PAR`` indices -- the caller ``labelset``s them (once) before
        calling. See :meth:`Line2D.__call__`."""
        return self._run(seq, pe_idx, par_idx, rf_phase_rad, rotation)
