...
class _BssfpReadout(SequenceModule):
    ...
    def init_module(
        ...
        *,
        ...
        gx_rew = bridge(
            system, "x", -amplitude * (flat_span - 0.5 * readout_duration), amplitude, 0.0
        )
        ...
        prep_span = 0.5 * (flat_span + (flat_span - readout_duration)) + rf_center - excite_span
        ...
        ramp = pp.ceil_to_raster(system.max_grad / system.max_slew, system.grad_raster_time)
        ...
        rewind_span += pp.round_to_raster(0.5 * solve_delay(tr, tr_min, "TR", system), raster)
        encode_span = rewind_span + offset

        gy_pre = pp.make_phase_encoding("y", fov[1] / n[1], system=system, duration=encode_span)
        gy_rew = pp.scale_grad(
            pp.make_phase_encoding("y", fov[1] / n[1], system=system, duration=rewind_span), -1.0
        )
        ...
        adc = pp.make_adc(num_samples=n_samples, dwell=dwell, delay=encode_span, system=system)
