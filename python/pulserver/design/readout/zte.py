    ...
    def init_module(
        ...
        *,
        ...
        if directions is None:
            ...
            n_views = max(3, -(-total // int(n_shots))) if n_views is None else int(n_views)
        ...
        dead_time_s = system.adc_dead_time if dead_time_s is None else float(dead_time_s)
        ...
        adc = pp.make_adc(num_samples=n_samples, dwell=dwell, delay=adc_delay, system=system)
        read_span = pp.ceil_to_raster(adc_delay + n_samples * dwell, system.grad_raster_time)
        ...
        tr_min = pp.ceil_to_raster(
            hold_span + read_span + max(step_span, ramp_span), system.block_duration_raster
        )
...
def _unit(directions: Any) -> np.ndarray:
    ...
    if values.ndim != 2 or values.shape[1] != 3 or len(values) < 2:
        raise ValueError("directions must have shape (views, 3) with at least two views")
...
def _view_rotations(directions: np.ndarray, tolerance: float = 1e-8):
    ...
    rotations.append(
        _minimal_turn(np.array([1.0, 0.0, 0.0]), turning[-1]).as_matrix()
    )
    matrices = np.asarray([Rotation.from_matrix(turn).as_matrix() for turn in rotations])
