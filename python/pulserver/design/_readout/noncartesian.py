...
def _as_event_tuple(events):
    """Normalize ``None`` / one event / an iterable of them into a tuple.

    Lets ``slice_rephasing=excitation.rephasers`` read the same here as it
    does on the Cartesian trains, without forcing callers to index a
    one-element tuple.
    """
    if events is None:
        return ()
    if hasattr(events, "channel"):
        return (events,)
...
class _NonCartesianTrain(Readout):
    def __init__(
        ...
        *,
        ...
        self.slice_rephasing = _as_event_tuple(slice_rephasing)
