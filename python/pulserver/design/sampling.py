"""K-space sampling patterns and view-ordering helpers.
...
def chunk_indices(indices: list[int], size: int) -> list[list[int]]:
    ...
    >>> sampling.chunk_indices([0, 1, 2, 3, 4], 2)
    [[0, 1], [2, 3], [4]]
    """
    size = max(1, int(size))
...
def outer_inner_order(outer_indices: list[int], inner_len: int) -> list[list[tuple[int, int]]]:
    ...
    --------
    >>> from pulserver.design import sampling
    >>> sampling.outer_inner_order([0, 1], 2)
    [[(0, 0), (0, 1)], [(1, 0), (1, 1)]]
    """
...
def fse_linear_order(coords, etl: int) -> list[list[int]]:
    ...
    --------
    >>> from pulserver.design import sampling
    >>> shots = sampling.fse_linear_order([[0, 0], [1, 0], [0, 1], [1, 1]], 2)
...
def fse_radial_order(coords, etl: int) -> list[list[int]]:
    ...
    >>> shots = sampling.fse_radial_order(coords, 5)
...
def fse_radial_adaptive_order(coords, etl: int, *, n_sections: int = 3) -> list[list[int]]:
    ...
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> shots = sampling.fse_radial_adaptive_order(coords, 7, n_sections=3)
...
def fse_shuffling_order(coords, etl: int, *, seed: int | None = None, cluster: bool = True) -> list[list[int]]:
    ...
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> a = sampling.fse_shuffling_order(coords, 8, seed=0)
    >>> b = sampling.fse_shuffling_order(coords, 8, seed=0)
    >>> a == b
    True
    """
    pts = _as_coords(coords)
    ...
    if cluster:
        # Grid-strip clustering: sort by kz then ky and cut into contiguous
        # trains so each train covers a spatially compact k-space region.
        base_order = sorted(range(n), key=lambda i: (pts[i, 1], pts[i, 0]))
    else:
        base_order = list(rng.permutation(n))
...
def random_mask(
    ...
    *,
    ...
    --------
    >>> from pulserver.design import sampling
    >>> mask = sampling.random_mask((32, 32), 4.0, calib=(8, 8), seed=0)
    >>> mask.shape
    (32, 32)
    """
    if accel <= 1:
...
def caipirinha_mask(
    ...
    *,
    ...
    --------
    >>> from pulserver.design import sampling
    >>> mask = sampling.caipirinha_mask((8, 8), 2, 2, delta=1)
    >>> int(mask.sum())
    16
    """
    if ry < 1 or rz < 1:
...
def poisson_disc_mask(
    ...
    *,
    ...
    --------
    >>> from pulserver.design import sampling
    >>> mask = sampling.poisson_disc_mask((48, 48), 4.0, calib=(8, 8), seed=1)
    >>> mask.shape
    (48, 48)
    """
    if accel <= 1:
    ...
    ny, nx = shape
    y, x = np.mgrid[:ny, :nx]
    x = np.maximum(abs(x - nx / 2) - calib[-1] / 2, 0)
    x = x / x.max()
    y = np.maximum(abs(y - ny / 2) - calib[-2] / 2, 0)
    y = y / y.max()
    r = np.sqrt(x**2 + y**2)

    slope_max = float(max(nx, ny))
    slope_min = 0.0
    mask = np.zeros(shape, dtype=bool)
    actual_accel = np.inf
    while slope_min < slope_max:
        ...
        mask = _poisson_bridson(nx, ny, max_attempts, radius_x, radius_y, calib, seed)
        if crop_corner:
            mask &= r < 1
        actual_accel = nx * ny / max(1, int(mask.sum()))
...
def _poisson_bridson(nx, ny, max_attempts, radius_x, radius_y, calib, seed):
    """Bridson dart-throwing core, ported from sigpy's numba ``_poisson``."""
    ...
    pxs[0] = rng.randint(0, nx)
    pys[0] = rng.randint(0, ny)
    num_actives = 1
    while nx * ny > num_actives > 0:
        i = rng.randint(0, num_actives)
        px, py = pxs[i], pys[i]
        rx, ry_ = radius_x[py, px], radius_y[py, px]

        done = False
        k = 0
        qx = qy = 0.0
        while not done and k < max_attempts:
            v = (rng.random_sample() * 3 + 1) ** 0.5
            t = 2 * np.pi * rng.random_sample()
            qx = px + v * rx * np.cos(t)
            qy = py + v * ry_ * np.sin(t)

            if 0 <= qx < nx and 0 <= qy < ny:
                startx = max(int(qx - rx), 0)
                endx = min(int(qx + rx + 1), nx)
                starty = max(int(qy - ry_), 0)
                endy = min(int(qy + ry_ + 1), ny)

                ys, xs = np.mgrid[starty:endy, startx:endx]
                neighborhood = mask[starty:endy, startx:endx]
                dist = ((qx - xs) / radius_x[starty:endy, startx:endx]) ** 2 + (
                    (qy - ys) / radius_y[starty:endy, startx:endx]
                ) ** 2
                done = not np.any(neighborhood & (dist < 1))
            k += 1

        if done:
            pxs[num_actives] = int(qx)
            pys[num_actives] = int(qy)
            mask[int(qy), int(qx)] = True
            num_actives += 1
        else:
...
def golden_angles(n: int) -> np.ndarray:
    ...
    Thin wrapper over :func:`pulserver.arbgrad.shot_angles` with the
    ...
    >>> import numpy as np
    >>> from pulserver.design import sampling
    >>> angles = sampling.golden_angles(4)
