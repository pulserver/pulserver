...
def calc_sampled_lines(n: int, r: int, acs_lines: int) -> list[int]:
    ...
    >>> _lowlevel.calc_sampled_lines(8, 2, 0)
    [0, 2, 4, 6]
    """
...
def make_linear_order(coords, train_length: int) -> list[list[int]]:
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> shots = _lowlevel.make_linear_order([[0, 0], [1, 0], [0, 1], [1, 1]], 2)
    ...
    .. plot::
       ...
       order_figure([("linear", _lowlevel.make_linear_order(coords, 32))], coords)
    """
...
def make_centric_order(coords, train_length: int) -> list[list[int]]:
    ...
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> shots = _lowlevel.make_centric_order(coords, 5)
    ...
    .. plot::
       ...
       ky, kz = np.meshgrid(np.arange(-16, 16), np.arange(-16, 16))
       coords = np.column_stack([ky.ravel(), kz.ravel()])
       order_figure([("centric", _lowlevel.make_centric_order(coords, 32))], coords)
...
def make_radial_order(coords, train_length: int) -> list[list[int]]:
    ...
    >>> ky, kz = np.meshgrid(np.arange(-2, 3), np.arange(-2, 3))
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> shots = _lowlevel.make_radial_order(coords, 5)
    >>> all(len(s) <= 5 for s in shots)
    True
    ...
    .. plot::
       ...
       ky, kz = np.meshgrid(np.arange(-16, 16), np.arange(-16, 16))
       coords = np.column_stack([ky.ravel(), kz.ravel()])
       order_figure([("radial", _lowlevel.make_radial_order(coords, 32))], coords)
...
def make_radial_adaptive_order(coords, train_length: int, *, n_sections: int = 3) -> list[list[int]]:
    ...
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> shots = _lowlevel.make_radial_adaptive_order(coords, 7, n_sections=3)
    >>> len(shots)
    7
    ...
    .. plot::
       ...
       ky, kz = np.meshgrid(np.arange(-16, 16), np.arange(-16, 16))
       coords = np.column_stack([ky.ravel(), kz.ravel()])
       order_figure([("radial adaptive", _lowlevel.make_radial_adaptive_order(coords, 32))], coords)
    """
    pts = _as_coords(coords)
    n = len(pts)
    if n == 0:
...
def make_shuffling_order(
    ...
    >>> coords = np.column_stack([ky.ravel(), kz.ravel()])
    >>> a = _lowlevel.make_shuffling_order(coords, 8, seed=0)
    >>> b = _lowlevel.make_shuffling_order(coords, 8, seed=0)
    >>> a == b
    True
    ...
    .. plot::
       ...
       ky, kz = np.meshgrid(np.arange(-16, 16), np.arange(-16, 16))
       coords = np.column_stack([ky.ravel(), kz.ravel()])
       order_figure([("T2 shuffling", _lowlevel.make_shuffling_order(coords, 32, seed=0))], coords)
    """
    pts = _as_coords(coords)
    n = len(pts)
    if n == 0:
...
def make_random_mask(
    ...
    *,
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> mask = _lowlevel.make_random_mask((32, 32), 4.0, calib=(8, 8), seed=0)
    >>> mask.shape
    (32, 32)
    ...
    .. plot::
       ...
       masks = [
           ("random, R=4", _lowlevel.make_random_mask((64, 64), 4.0, calib=(12, 12), seed=0)),
           ("poisson-disc, R=4", _lowlevel.make_poisson_disc_mask((64, 64), 4.0, calib=(12, 12), seed=1)),
           ("CAIPI 2x2, delta=1", _lowlevel.make_caipirinha_mask((64, 64), 2, 2, delta=1)),
       ]
...
def make_caipirinha_mask(
    ...
    *,
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> mask = _lowlevel.make_caipirinha_mask((8, 8), 2, 2, delta=1)
    >>> int(mask.sum())
    16
    ...
    .. plot::
       ...
       for ax, delta in zip(axes, (0, 1, 2)):
           ax.imshow(_lowlevel.make_caipirinha_mask((32, 16), 2, 3, delta=delta).T,
...
def make_poisson_disc_mask(
    ...
    *,
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> mask = _lowlevel.make_poisson_disc_mask((48, 48), 4.0, calib=(8, 8), seed=1)
    >>> mask.shape
    (48, 48)
    ...
    .. plot::
       ...
       for ax, accel in zip(axes, (2.0, 4.0, 8.0)):
           ax.imshow(_lowlevel.make_poisson_disc_mask((64, 64), accel, calib=(12, 12), seed=1).T,
                     cmap="gray", origin="lower", interpolation="nearest")
