...
def calc_golden_angles(n: int) -> np.ndarray:
    ...
    >>> np.rad2deg(_lowlevel.calc_golden_angles(4)).round(2)
    ...
    .. plot::
       ...
       import pulserver.pypulseq as pp
       schemes = [
           ("uniform", _lowlevel.calc_uniform_angles(34)),
           ("golden", _lowlevel.calc_golden_angles(34)),
           ("tiny golden, N=4", _lowlevel.calc_tiny_golden_angles(34, index=4)),
           ("RAGA", _lowlevel.calc_raga_angles(34, approximation_order=9)),
       ]
...
def calc_raga_angles(n: int, *, tiny_index: int = 1, approximation_order: int = 13) -> np.ndarray:
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> angles = _lowlevel.calc_raga_angles(1000, approximation_order=8)
...
def calc_tiny_golden_angles(n: int, *, index: int = 2) -> np.ndarray:
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> np.rad2deg(_lowlevel.calc_tiny_golden_angles(3, index=2)).round(2)
...
def calc_uniform_angles(n: int) -> np.ndarray:
    ...
    >>> from pulserver.design import _lowlevel
    >>> import pulserver.pypulseq as pp
    >>> np.rad2deg(_lowlevel.calc_uniform_angles(4))
    array([  0.,  90., 180., 270.])
