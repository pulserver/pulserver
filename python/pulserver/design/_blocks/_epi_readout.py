...
def make_epi_readout(
    ...
    gblip, _, _ = pp.make_extended_trapezoid_area(channel='y', area=ky_blip_area, grad_start=0.0, grad_end=0.0)
    gblip.delay = blip_delay
    
    # Build prewinder
    gx_prew, _, _ = pp.make_extended_trapezoid_area(channel='x', area=-0.5 * gread.area, grad_start=0.0, grad_end=0.0)
    gx_rew = pp.scale_grad(gx_prew, scale=(-1)**(num_readouts_per_shot+1))
  
    gy_prew, _, _ = pp.make_extended_trapezoid_area(channel='y', area=-ky_area, grad_start=0.0, grad_end=0.0)
