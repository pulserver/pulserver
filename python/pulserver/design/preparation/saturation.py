    ...
    **The two arms are matched on power, not on amplitude.** Deposited power
    ...
    def init_module(
        ...
        **kwargs: Any,
        ...
        if freq_offset_hz <= 0:
            raise ValueError("freq_offset_hz must be positive; both signs are built from it")
        if duration_s <= 0 or time_bw_product <= 0:
            raise ValueError("duration_s and time_bw_product must be positive")
...
class BlochSiegertPreparation(OffResonanceSaturation):
    ...
    def init_module(
        ...
        **kwargs: Any,
        ...
        if freq_offset_hz == 0:
            raise ValueError("freq_offset_hz must not be zero: the shift is inversely in it")
        ...
        self.kbs_per_gauss2 = float(
            np.sum((2.0 * np.pi * gamma_hz_per_gauss * envelope / peak_b1_hz) ** 2 / denominator)
            * dwell_s
        )
