...
| --- | ---: | ---: | ---: |
| GRE 64² × 1 slice | 512 | 34 | 15× |
| GRE 128² × 16 slices | 14 464 | 48 | 301× |
| GRE 256² × 32 slices | 57 600 | 54 | 1067× |
| FSE 256² × 20 slices, ETL 16 | 22 080 | 105 | 210× |
| bSSFP 128², 64 frames | 16 576 | 43 | 386× |
| MPRAGE 192² × 128 partitions | 181 248 | 1 112 | 163× |

Note that the unique-block count is almost flat as the matrix grows: GRE goes
from 512 blocks to 57 600 — 112× more sequence — for 34 → 54 definitions.
Phase encoding does not create new definitions, because a scaled gradient is
the same definition at a different per-instance amplitude. That is why the
scanner-side memory in the tables further down barely moves with matrix size.
