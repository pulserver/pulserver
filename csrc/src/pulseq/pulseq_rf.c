 ...
 * transform.
 */
static void interp_linear(PULSEQ_REAL *out, const PULSEQ_REAL *x, int nx, const PULSEQ_REAL *xp,
                          const PULSEQ_REAL *fp, int np)
{
    int i, j = 0;
 ...
 */
static double flank(const PULSEQ_REAL *freq, const PULSEQ_REAL *re, const PULSEQ_REAL *im, int n,
                    double level, int from_the_end)
{
    int i, step, first;
...
int pulseq_rf_spectrum_create(pulseq_rf_spectrum **out, PULSEQ_REAL raster_us,
                              PULSEQ_REAL resolution_hz)
{
    ...
    *out = NULL;
    ...
    if (!plan->t || !plan->freq || !plan->re || !plan->im || !plan->in || !plan->out ||
        !plan->cfg)
    {
...
int pulseq_rf_spectrum_run(pulseq_rf_spectrum *plan, const PULSEQ_REAL *re, const PULSEQ_REAL *im,
                           const PULSEQ_REAL *t_us, int count, PULSEQ_REAL center_us)
{
...
PULSEQ_REAL pulseq_rf_bandwidth(const pulseq_rf_spectrum *plan, PULSEQ_REAL cutoff,
                                PULSEQ_REAL *out_center_hz)
{
    ...
    for (i = 0; i < plan->n; ++i)
    {
        double m = sqrt((double)plan->re[i] * (double)plan->re[i] +
                        (double)plan->im[i] * (double)plan->im[i]);
        if (m > peak)
            peak = m;
    }
    if (!(peak > 1.0e-12))
