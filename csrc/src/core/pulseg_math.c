/* pulseg_math.c -- math, interpolation, and FFT helpers */
...
void pulseg__mag_phase_to_real_imag(
    float* re, float* im,
...
float pulseg__trapz_complex_mag_uniform(
...
float pulseg__trapz_complex_mag_nonuniform(
...
void pulseg__apply_rotation(float* out, const float* R, const float* v,
                               int transpose)
{
    if (transpose) {
    ...
    const float* kernel, int kernel_len)
{
    ...
    if (!pad_sig || !pad_kern || !sig_fft || !kern_fft || !conv) {
        result = PULSEG_ERR_ALLOC_FAILED; goto fail;
    }
    ...
    if (!fwd || !inv) { result = PULSEG_ERR_PNS_FFT_FAILED; goto fail; }
...
fail:
    if (pad_sig)  PULSEG_FREE(pad_sig);
    if (pad_kern) PULSEG_FREE(pad_kern);
    if (sig_fft)  PULSEG_FREE(sig_fft);
    if (kern_fft) PULSEG_FREE(kern_fft);
    if (conv)     PULSEG_FREE(conv);
