 ...
 * either. */
static void pns_memo_accumulate(float *dst, int n, const float *src, int src_len, int offset, float scale)
{
    int j, j0, lim;
    ...
    int pad)
{
    ...
    *applied = 0;
    ...
    if (!pns_memo_block_offsets(
            desc, block_start, block_count, block_order, uw->raster_us, n_uniform, offsets, lengths))
    ...
    for (i = 0; i < num_occurrences; ++i)
    {
        ...
        pns_memo_accumulate(
            axis_out[tp->axis], n, tp->resp, tp->resp_len, occurrences[i].offset, occurrences[i].scale);
    }
    ...
    for (i = 0; i < num_occurrences; ++i)
    {
        ...
        if (off >= n)
            continue;
        pns_memo_accumulate(
            axis_out[tp->axis], n, tp->resp, tp->resp_len, off, occurrences[i].scale);
    }
