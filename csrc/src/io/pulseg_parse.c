/* pulseg_parse.c -- .seq file parsing, library init/read, block retrieval */
    ...
    if (seq->is_definitions_library_parsed && seq->definitions_library)
    {
        for (i = 0; i < seq->num_definitions; i++)
        {
            for (j = 0; j < seq->definitions_library[i].value_size; j++)
                PULSEG_FREE(seq->definitions_library[i].value[j]);
            PULSEG_FREE(seq->definitions_library[i].value);
        }
        PULSEG_FREE(seq->definitions_library);
    }
    if (seq->is_block_library_parsed)
    {
        PULSEG_FREE(seq->block_library);
        PULSEG_FREE(seq->block_ids);
        seq->block_ids = NULL;
    }
    if (seq->is_rf_library_parsed)
    {
        PULSEG_FREE(seq->rf_library);
        if (seq->rf_use_tags)
            PULSEG_FREE(seq->rf_use_tags);
        seq->rf_use_tags = NULL;
    }
    if (seq->is_grad_library_parsed)
        PULSEG_FREE(seq->grad_library);
    if (seq->is_adc_library_parsed)
        PULSEG_FREE(seq->adc_library);
    if (seq->is_extensions_library_parsed)
    {
        PULSEG_FREE(seq->extensions_library);
        PULSEG_FREE(seq->trigger_library);
        PULSEG_FREE(seq->rotation_quaternion_library);
        PULSEG_FREE(seq->rotation_matrix_library);
        PULSEG_FREE(seq->labelset_library);
        PULSEG_FREE(seq->labelinc_library);
        PULSEG_FREE(seq->soft_delay_library);
        PULSEG_FREE(seq->rf_shim_library);
    }
    if (seq->is_shapes_library_parsed && seq->shapes_library)
    {
        for (i = 0; i < seq->shapes_library_size; i++)
        {
            PULSEG_FREE(seq->shapes_library[i].samples);
            seq->shapes_library[i].samples = NULL;
            seq->shapes_library[i].num_uncompressed_samples = 0;
            seq->shapes_library[i].num_samples = 0;
        }
        PULSEG_FREE(seq->shapes_library);
    }
    PULSEG_FREE(seq->extension_lut);
    seq->extension_lut = NULL;
...
void pulseg_pulseq_file_set_free(pulseg_pulseq_file_set *coll)
{
    ...
    if (coll->sequences)
    {
        for (i = 0; i < coll->num_sequences; ++i)
        {
            ...
            {
                PULSEG_FREE(coll->sequences[i].file_path);
                coll->sequences[i].file_path = NULL;
            }
        }
        PULSEG_FREE(coll->sequences);
    }
    if (coll->base_path)
                                 ...
                                 void **target, int *target_count, int n)
{
    ...
    arr = (float *)PULSEG_ALLOC(sizeof(float) * n * max_idx);
    if (!arr)
        return 1;
                                    ...
                                    pulseg__definition **target, int *target_count)
{
    ...
    defs = (pulseg__definition *)PULSEG_ALLOC(sizeof(pulseg__definition) * count);
    if (!defs)
        return 1;
    *target = defs;
    *target_count = count;
                               ...
                               pulseg_shape_arbitrary **target, int *target_count)
{
    ...
    shapes = (pulseg_shape_arbitrary *)PULSEG_ALLOC(sizeof(pulseg_shape_arbitrary) * max_idx);
    if (!shapes)
        return 1;
    for (i = 0; i < max_idx; i++)
    {
    ...
    for (i = 0; i < max_idx; i++)
    {
        ...
        if (n > 0)
        {
            shapes[i].samples = (float *)PULSEG_ALLOC(sizeof(float) * n);
            if (!shapes[i].samples)
            {
                for (j = 0; j < i; j++)
                {
                    if (shapes[j].samples)
                        PULSEG_FREE(shapes[j].samples);
                }
                PULSEG_FREE(shapes);
                return 1;
            }
                                ...
                                pulseg__rf_shim_entry **target, int *target_count)
{
    ...
    arr = (pulseg__rf_shim_entry *)PULSEG_ALLOC(sizeof(pulseg__rf_shim_entry) * max_idx);
    if (!arr)
        return 1;
    for (i = 0; i < max_idx; i++)
...
static void read_definitions_library(pulseg_pulseq_file *seq, FILE *f)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        {
            new_array = (char **)PULSEG_ALLOC(sizeof(char *) * (def.value_size + 1));
            for (i = 0; i < def.value_size; i++)
                new_array[i] = def.value[i];
            new_array[def.value_size] = (char *)PULSEG_ALLOC(strlen(token) + 1);
    ...
    if (seq->rf_library_size > 0 && seq->offsets.rf >= 0)
    {
        ...
        if (seq->rf_use_tags)
        {
            ...
            for (ui = 0; ui < seq->rf_library_size; ++ui)
                seq->rf_use_tags[ui] = PULSEG_RF_USE_UNKNOWN;
                ...
                fgets(use_line, sizeof(use_line), f))
            {
                while (fgets(use_line, sizeof(use_line), f))
                {
                    ...
                    {
                        char tag = *scan_p;
                        int use_val = PULSEG_RF_USE_UNKNOWN;
                        if (tag == 'e')
                            use_val = PULSEG_RF_USE_EXCITATION;
                        else if (tag == 'r')
                            use_val = PULSEG_RF_USE_REFOCUSING;
                        else if (tag == 'i')
                            use_val = PULSEG_RF_USE_INVERSION;
                        else if (tag == 's')
                            use_val = PULSEG_RF_USE_SATURATION;
                              ...
                              int n, int *is_label_defined)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (idx <= 0 || idx > target_count)
            continue;
        label_code = pulseg__label2enum(label);
        if (label_code > 0)
                              ...
                              int n, int *is_delay_defined)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        if (idx <= 0 || idx > target_count)
            continue;
        hint_code = pulseg__hint2enum(hint);
        if (hint_code > 0)
...
static void read_extensions_library(pulseg_pulseq_file *seq, FILE *f)
{
    ...
    pulseg__scale ext_s, trig_s, rot_s;
    ...
    {
        seq->extension_lut = (int *)PULSEG_ALLOC(sizeof(int) * (seq->extension_lut_size + 1));
        for (n = 0; n < 8; n++)
        {
                                ...
                                const pulseg_shape_arbitrary *encoded, float scale)
{
    ...
    if (encoded->num_samples == encoded->num_uncompressed_samples)
    {
        ...
        result->samples = (float *)PULSEG_ALLOC(sizeof(float) * encoded->num_samples);
        if (!result->samples)
            return 0;
    ...
    unpacked = (float *)PULSEG_ALLOC(sizeof(float) * num_samples);
    if (!unpacked)
        return 0;
...
void pulseg_opts_init_full(pulseg_opts *opts,
...
void pulseg_pulseq_block_free(pulseg_pulseq_block *block)
{
    ...
    if (block->gx.type > 1)
    {
        ...
        {
            PULSEG_FREE(block->gx.wave_shape.samples);
            block->gx.wave_shape.samples = NULL;
        }
        if (block->gx.time_shape.samples)
        {
            PULSEG_FREE(block->gx.time_shape.samples);
    ...
    if (block->gy.type > 1)
    {
        ...
        {
            PULSEG_FREE(block->gy.wave_shape.samples);
            block->gy.wave_shape.samples = NULL;
        }
        if (block->gy.time_shape.samples)
        {
            PULSEG_FREE(block->gy.time_shape.samples);
    ...
    if (block->gz.type > 1)
    {
        ...
        {
            PULSEG_FREE(block->gz.wave_shape.samples);
            block->gz.wave_shape.samples = NULL;
        }
        if (block->gz.time_shape.samples)
        {
            PULSEG_FREE(block->gz.time_shape.samples);
    ...
    if (block->adc.type > 0)
    {
        ...
        {
            PULSEG_FREE(block->adc.phase_modulation_shape.samples);
    ...
    if (block->rf_shimming.type > 0)
    {
        ...
        {
            PULSEG_FREE(block->rf_shimming.amplitudes);
            block->rf_shimming.amplitudes = NULL;
        }
        if (block->rf_shimming.phases)
        {
            PULSEG_FREE(block->rf_shimming.phases);
...
int pulseg_pulseq_file_read_from_buffer(pulseg_pulseq_file *seq, FILE *f)
{
    ...
    if (seq->version_combined < 1005000)
        return PULSEG_ERR_UNSUPPORTED_VERSION;
...
static int count_sequences_in_chain(const char *first_path, const pulseg_opts *opts)
{
    int count = 0;
    int max_count = 1000;
    char *current_path;
    ...
    if (!base_path)
        return -1;
    current_path = (char *)PULSEG_ALLOC(strlen(first_path) + 1);
    if (!current_path)
    {
                                   ...
                                   const pulseg_opts *opts)
{
    ...
    if (num_seq <= 0)
    {
        ...
        coll->sequences = NULL;
        coll->base_path = NULL;
        return (num_seq == 0) ? PULSEG_ERR_COLLECTION_EMPTY : PULSEG_ERR_COLLECTION_CHAIN_BROKEN;
    }
    ...
    current_path = (char *)PULSEG_ALLOC(strlen(first_file_path) + 1);
    if (!current_path)
    {
        PULSEG_FREE(coll->sequences);
        PULSEG_FREE(coll->base_path);
    ...
    for (i = 0; i < num_seq; ++i)
    {
        ...
        coll->sequences[i].file_path = (char *)PULSEG_ALLOC(strlen(current_path) + 1);
        if (coll->sequences[i].file_path)
...
static void extension_block_init(pulseg__extension_block *eb)
{
    if (!eb)
        return;
...
static void extension_block_free(pulseg__extension_block *eb)
{
    if (!eb)
        return;
    if (eb->rf_shimming.amplitudes)
    {
        PULSEG_FREE(eb->rf_shimming.amplitudes);
        eb->rf_shimming.amplitudes = NULL;
    }
    if (eb->rf_shimming.phases)
    {
        PULSEG_FREE(eb->rf_shimming.phases);
        eb->rf_shimming.phases = NULL;
    }
    eb->rf_shimming.type = 0;
    eb->rf_shimming.num_channels = 0;
                                  ...
                                  const pulseg_pulseq_raw_extension *re)
{
    ...
    phs = (float *)PULSEG_ALLOC(sizeof(float) * n);
    if (!amps || !phs)
    {
        if (amps)
            PULSEG_FREE(amps);
    ...
    if (raw->rf >= 0 && seq->rf_library && raw->rf < seq->rf_library_size)
    {
        ...
        if (idx > 0 && seq->is_shapes_library_parsed && idx <= seq->shapes_library_size)
        {
            ...
            for (i = 0; i < block->rf.phase_shape.num_samples; i++)
                block->rf.phase_shape.samples[i] *= (float)PULSEG__TWO_PI;
        }
        else
        {
        ...
        {
            is_real = (int *)PULSEG_ALLOC(block->rf.mag_shape.num_samples * sizeof(int));
            if (!is_real)
                goto fail;
...
int pulseg_pulseq_verify_signature(const char *file_path)
{
    ...
    {
        fclose(f);
        return PULSEG_ERR_SIGNATURE_MISSING;
    }
    ...
    if (strcmp(computed, stored_hash) != 0)
        return PULSEG_ERR_SIGNATURE_MISMATCH;
