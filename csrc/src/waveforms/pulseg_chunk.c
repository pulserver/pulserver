    ...
    if (scale > 0.0f)
    {
        for (axis = 0; axis < 3; ++axis)
        {
            ...
            if (ratio_tolerance > 0.0f)
                out->ratio_q[axis] = (int)(out->ratio[axis] / ratio_tolerance +
                                           (amp[axis] >= 0.0f ? 0.5f : -0.5f));
        }
    }
    return 1;
}
...
static int intern_wave(
    pulseg_wave_key **waves, int *count, int *cap, const pulseg_wave_key *key)
{
    int i;
    ...
    pulseg_diagnostic *diag)
{
    ...
    for (pos = 0; pos < desc->exec_stream_len; ++pos)
    {
        ...
        if (!starts_instance && num_chunks > 0 && seg_id >= 0 &&
            seg_id < desc->num_unique_segments)
        {
        ...
        for (k = 1; k < num_chunks; ++k)
        {
            ...
            if (chunks[k].build_us > allowed)
            {
                if (diag)
                {
                    ...
                    pulseg__diag_printf(
                        diag, " segment_chunk=%d build_us=%.0f allowed_us=%.0f", k,
                        (double)chunks[k].build_us, (double)allowed);
                    pulseg__diag_printf(
                        diag, " waves=%d points=%ld", chunks[k].num_waves,
    ...
    axis_corners *out)
{
    ...
    if (gd->unused_or_time_shape_id > 0 && gd->unused_or_time_shape_id <= desc->num_shapes)
    {
        if (pulseq_decompress_shape(
                &decomp_t, &desc->shapes[gd->unused_or_time_shape_id - 1], raster))
...
static int wave_needs_centre_raster(
    const pulseg_sequence_descriptor *desc, const pulseg_wave_key *key)
{
    const pulseg_grad_definition *gd;
    int d;
    ...
    float *out_peak)
{
    ...
    *out_num_points = n_union;
    ...
    if (key->rotation_id >= 0 && key->rotation_id < desc->num_rotations &&
