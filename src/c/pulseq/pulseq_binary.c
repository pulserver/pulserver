...
static int read_binary_adc(bin_reader *r, pulseq_file *seq)
{
    ...
    /* Dense ids are the norm, so one allocation covers the section. */
    rc =
        lib_reserve((PULSEQ_REAL **)&seq->adc_library, &cap, &seq->adc_library_size, 8, (int)count);
    if (PULSEQ_FAILED(rc))
        return rc;
...
static int read_binary_labels(bin_reader *r, pulseq_file *seq, int is_inc)
{
    ...
    int cap = 0;
    PULSEQ_REAL **rows =
        is_inc ? (PULSEQ_REAL **)&seq->labelinc_library : (PULSEQ_REAL **)&seq->labelset_library;
