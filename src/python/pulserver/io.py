...
def _render_int_rows(matrix: np.ndarray, widths: tuple[int, ...]) -> str | None:
    ...
    spans = [
        max(width, len(str(int(value))))
        for width, value in zip(widths, maxima, strict=True)
    ]
...
def write(
    ...
    *,
    ...
    if n_blocks:
        ...
        ticks = np.fromiter(
            target.block_durations.values(), dtype=float, count=n_blocks
        )
        ...
        printed[:, 0] = np.fromiter(
            target.block_durations, dtype=np.int64, count=n_blocks
        )
        printed[:, 1] = rounded
    ...
    if len(target.rf_library.data) != 0:
        ...
        for k, data in target.rf_library.data.items():
            ...
            use_char = (
                _RF_USE_CODE_TO_CHAR.get(int(use_type), "u")
                if isinstance(use_type, int | np.integer)
                else use_type
            )
    ...
    grad_keys = (
        np.array(list(target.grad_library.data.keys()), dtype=int)
        if target.grad_library.data
        else np.array([])
    )
    grad_types = (
        np.array([target.grad_library.type[k] for k in grad_keys], dtype=object)
        if len(grad_keys)
        else np.array([])
    )
    ...
    if np.any(trap_grad_mask):
        ...
        for k in grad_keys[trap_grad_mask].tolist():
            ...
            append(
                trap_fmt
                % (k, amplitude, 1e6 * rise, 1e6 * flat, 1e6 * fall, 1e6 * delay)
            )
        w("\n")
    ...
    if len(target.label_set_library.data) != 0:
        ...
        for k, data in target.label_set_library.data.items():
            ...
            append(
                label_fmt % (k, data[0], registry.get(label_num, f"CUSTOM_{label_num}"))
            )
        w("\n")
    ...
    if len(target.rf_shim_library.data) != 0:
        ...
        for k in target.rf_shim_library.data:
            ...
            s = id_format_str.format(
                k, int(0.5 * shim_vector_length), *target.rf_shim_library.data[k]
            )
            w(s)
        w("\n")
