...
def _block_table(seq) -> tuple[np.ndarray, np.ndarray]:
    ...
    if deferred is not None:
        ...
        count = len(events)
        table = (
            np.stack(list(events.values()))
            if count
            else np.zeros((0, 7), dtype=np.int64)
        )
        durations = np.fromiter(
            seq.block_durations.values(), dtype=np.float64, count=count
        )
...
def _set_definition(native, key: str, value) -> None:
    ...
    native.set_definition_numbers(
        key, [float(item) for item in values.ravel()], integers
    )
...
def to_native(seq, *, rotation_library=None, rf_shim_library=None, label_names=None):
    ...
    source = (
        getattr(seq, "_seq", seq)
        if getattr(seq, "_deferred", None) is not None
        else seq
    )
    ...
    native.set_version(
        int(source.version_major),
        int(source.version_minor),
        int(source.version_revision),
    )
    native.set_rasters(
    ...
    native.set_rf(
        rf,
        "".join(str(uses.get(key, "u"))[:1] or "u" for key in source.rf_library.data),
    )
...
def _label_name(label_id: int, registry=None) -> str:
    ...
    from .._ext.pulseqpp import Sequence as _NativeSequence
...
def to_upstream(
    seq, *, first: int = 1, last: int | None = None, numbers=None, lead_in=None
):
    ...
    upstream.block_durations = _ByBlock(
        numbers, np.ascontiguousarray(spans, dtype=np.float64)
    )
...
class _ByBlock(Mapping):
    ...
    def __init__(self, numbers: np.ndarray, values: np.ndarray) -> None:
        ...
        consecutive = (
            numbers.size > 0 and int(numbers[-1]) - int(numbers[0]) + 1 == numbers.size
        )
        self._base = int(numbers[0]) if consecutive else 0
        self._at = (
            None if consecutive else {int(n): i for i, n in enumerate(numbers.tolist())}
        )
...
def _take_extension_row(native, upstream, name: str, reference: int) -> bool:
    ...
    if native.is_custom_label(label_id):
        return False
    library = (
        upstream.label_set_library if name == "LABELSET" else upstream.label_inc_library
    )
...
class RestoringSequence(pp.Sequence):
    ...
    def waveforms(
        self, append_RF: bool = False, time_range=None
    ) -> tuple[np.ndarray, ...]:
    ...
    def _gradient_piece(self, gradient, elapsed: float, number: int):
            ...
            if abs(on_centres[0] - 1.0) < eps:
                ...
                return np.array(
                    [
                        ...
                        np.concatenate(
                            ([gradient.first], gradient.waveform, [gradient.last])
                        ),
                    ]
                )
        ...
        if abs(gradient.rise_time) > eps and abs(gradient.fall_time) > eps:
            return np.vstack(
                (
                    cumsum(
                        elapsed + gradient.delay, gradient.rise_time, gradient.fall_time
                    ),
