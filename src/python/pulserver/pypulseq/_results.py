    ...
    **The prescription is not in the sequence, so the tensor comes in three
    ...
    def compose(self, rotation: np.ndarray) -> np.ndarray:
        ...
        return (
            self.b_fixed
            + cross
            + np.swapaxes(cross, -1, -2)
            + R @ self.b_rotatable @ R.T
        )
    ...
    def of(
        ...
        *,
        ...
        leading = (
            np.argmax(np.abs(principal), axis=-1)
            if tensors.size
        ...
        with np.errstate(invalid="ignore", divide="ignore"):
            ...
            axial = (
                np.take_along_axis(
                    values,
                    np.argmax(np.abs(values - mean[:, None]), axis=-1)[:, None],
                    axis=-1,
                ).squeeze(-1)
                if tensors.size
                else np.zeros(0)
            ...
            delta = np.where(
                traces > 0, (axial - radial) / np.where(traces > 0, traces, 1.0), 0.0
            )
