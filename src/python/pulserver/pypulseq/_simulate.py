...
def _quat_multiply(q: np.ndarray, r: np.ndarray) -> np.ndarray:
    ...
    return np.concatenate(
        [
            qs * rs - np.sum(qv * rv, axis=1, keepdims=True),
            qs * rv + rs * qv + np.cross(qv, rv),
        ],
        axis=1,
    )
...
def bloch(b1_hz, bz_hz, dt: float, *, initial=None) -> np.ndarray:
    ...
    for step in range(len(b1_hz)):
        ...
        magnetisation = (
            ...
            + np.cross(axis, magnetisation) * sine
            + axis * dot * (1.0 - cosine)
        )
...
def _bandwidth_and_centre(
    rf, freq_offset: float, cutoff: float, df: float
) -> tuple[float, float]:
    ...
    return float(frequency[right] - frequency[left]), float(
        0.5 * (frequency[left] + frequency[right])
    )
...
def sim_rf(
    ...
    *,
    ...
    envelope = (
        ...
        * np.pi
        * signal
        * np.exp(1j * (phase_offset + 2.0 * np.pi * freq_offset * source_t))
    )
    shape = np.interp(
        times, source_t, envelope.real, left=0.0, right=0.0
    ) + 1j * np.interp(times, source_t, envelope.imag, left=0.0, right=0.0)
