...
class HostDaemon:
    ...
    a block for ``OPEN`` (limits, ``ir_`` conversion options and check
    limits), for ``VALIDATE`` and ``GENERATE`` (protocol values) and for
    ``IMPORT`` (the file). A session opened without a plugin only imports
    sequence files. Every revision holds ``sequence.seq`` and the IR cache the
    target loads. Replies:
    ...
    - ``IMPORT``: ``IMPORTED <revision>``. The file's ``NextSequence`` chain is
      copied into the revision; identical files at the same prescription
      return the revision holding them.

    A design is written in the logical frame. It is checked in the physical
    frame of the request's prescription rotation, and its IR cache is shifted
    to the request's field-of-view offset: the prescription entries of the
    protocol block for ``GENERATE``, the ``fov_offset_*`` and
    ``fov_rotation_*`` lines of the import block for ``IMPORT``.

    A generated or imported chain that fails a check of
    :func:`pulserver.ir.check` under the session's limits writes no revision,
    and ``GENERATE`` or ``IMPORT`` replies with its problems. A revision's
    identity includes the contents of the VOP file the limits name. ``OPEN``
    refuses limits that are not ``pypulseqpp.Opts`` arguments, conversion
    options or check limits. Any command can instead reply with a single
    ``ERROR <message>`` line. Commands of one session run one at a time;
    sessions run concurrently, with plugin code in a pool of spawned worker
    processes.
    ...
    async def _open(self, key: SessionKey, args: list[str], block: str) -> str:
        ...
        limits = parse_limits(block)
        try:
            _worker.split_limits(limits)
            self.store.open(key, plugin, limits)
    ...
    async def _generate(self, key: SessionKey, _args: list[str], block: str) -> str:
        ...
        async with self._locks[key]:
            ...
            digest = revision_hash(
                session.plugin, _identified(session.limits), validation.values, source
            )
    ...
    async def _import(self, key: SessionKey, _args: list[str], block: str) -> str:
        session = self.store.get(key)
        try:
            source, offset_mm, rotation = parse_import(block)
        ...
        async with self._locks[key]:
            ...
            digest = revision_hash(
                session.plugin, _identified(session.limits), identity
            )
            revision = session.find(digest)
            if revision is not None:
                session.select(revision)
            ...
            try:
                ...
                problems = await self._run(
                    _worker.check, session.limits, str(entry), rotation
                )
                if problems:
...
def _identified(limits: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise CommandError(f"cannot read the VOP file: {error}") from None
