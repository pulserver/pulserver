...
namespace pulseq
{
    ...
    KSpace calculate_kspace(const Sequence& seq, const KSpaceOptions& options)
    {
        ...
        for (int i = 0; i < num_readouts; ++i)
        {
            ...
            for (int a = 0; a < 3; ++a)
                r.constant_axes[static_cast<size_t>(a)] = (info.constant_axes & (1u << a)) != 0u;
        ...
        if (options.block_origins)
        {
            ...
            out.block_origins.resize(static_cast<size_t>((last >= first) ? (last - first + 1) : 0));
        ...
        if (options.dense)
        {
            ...
            dense.at_times(
                out.t_ktraj.data(),
                static_cast<int>(out.t_ktraj.size()),
                out.k_ktraj.data());
        }
