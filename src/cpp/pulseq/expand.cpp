    ...
    ExpandResult expand_repeats(Sequence& seq, int repeats, const ExpandOptions& options)
    {
        if (repeats < 1)
            throw std::invalid_argument(
                "expand_repeats(): repeats must be at least 1, got " + std::to_string(repeats));
        ...
        if (scan.counter_written)
            throw std::runtime_error(
                "expand_repeats(): the sequence already writes " + options.label +
        ...
        for (int b = 1; b <= n; ++b)
        {
            ...
            if (state < 0 || state > 2)
                throw std::runtime_error(
                    "expand_repeats(): block " + std::to_string(b) + " carries ONCE=" +
                    std::to_string(state) + "; ONCE is a three-state flag (0, 1 or 2)");
            if (state == 1)
                ++result.prep_blocks;
            else if (state == 2)
        ...
        if (options.strip_once && once_id != 0 && (labelset != 0 || labelinc != 0))
        {
            ...
            for (int b = 1; b <= n; ++b)
            {
                ...
                if (head != 0 && stripped.find(head) == stripped.end())
                    stripped[head] = strip_once_links(seq, head, once_id, labelset, labelinc);
            }
        }
                    ...
                    if (row == counter_rows.end())
                        row =
                            counter_rows
                                .emplace(
                                    value,
                                    seq.register_label_set(value, static_cast<int32_t>(counter_id)))
                                .first;
