...
namespace pulseq
{
    namespace
    {
         ...
         * hands it to the shared estimator in `src/c/pulseq/pulseq_rf.c`,
        ...
        double rf_bandwidth(const Sequence& seq, int block_index)
        {
            ...
            const std::vector<double> magnitude = decompress_shape(
                shapes.samples(mag_id),
                shapes.num_compressed(mag_id),
            ...
            if (time_id > 0)
            {
                ...
                for (int i = 0; i < n; ++i)
                    t[static_cast<size_t>(i)] =
                        static_cast<float>(tt[static_cast<size_t>(i)] * raster_us);
            }
            else
            {
                for (int i = 0; i < n; ++i)
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                const double p = phase.empty() ? 0.0 : 2.0 * M_PI * phase[static_cast<size_t>(i)];
            ...
            if (pulseq_rf_spectrum_create(
                    &plan,
                    static_cast<float>(raster_us),
            ...
            if (pulseq_rf_spectrum_run(
                ...
                bandwidth = pulseq_rf_bandwidth(
                    plan,
                    static_cast<float>(PULSEQ_RF_DEFAULT_CUTOFF),
                    nullptr);
         ...
         */
        std::vector<int> infer_repeat_sizes(
            ...
            size_t levels)
        {
            ...
            for (size_t p = 0; p < visits.size(); ++p)
            {
                if (p == 0)
                    ...
                    throw std::runtime_error(
                        ...
                        "was acquired " +
                        std::to_string(count) + " time(s) and another " +
            ...
            for (size_t p = 0; p < visits.size(); ++p)
            {
                ...
                for (size_t j = 1; j < visits[p].size(); ++j)
                    gaps.push_back(
                        static_cast<long>(visits[p][j]) - static_cast<long>(visits[p][j - 1]));
        ...
        const AutoLabelOptions& options)
    {
         ...
         * saying so before the work is done is cheaper for both of us. */
        {
            ...
            for (size_t d = 0; d < dims.size(); ++d)
            {
                ...
                for (size_t e = 0; e < d; ++e)
                {
                    if (dims[e].name == name)
                        throw std::runtime_error(
                            "auto_label: repeat dimension " + name + " is declared twice");
                }
                /* 0 means "read it from the acquisition order"; a negative
                 * number is not a size at all. */
                if (dims[d].size < 0)
                    throw std::runtime_error(
                        "auto_label: repeat dimension " + name + " has a negative size");
            }
        }
        ...
        const double s_first = (g_first[static_cast<size_t>(dominant(g_first))] < 0.0) ? -1.0 : 1.0;
        ...
        for (int i = first_signal; i < n_adc; ++i)
        {
            ...
            {
                const double d =
                    g[static_cast<size_t>(a)] * s - g_first[static_cast<size_t>(a)] * s_first;
                diff += d * d;
            }
            ...
            for (int j = 0; j < central_ro.num_samples; ++j)
            {
                ...
                for (int a = 0; a < 3; ++a)
                    k[static_cast<size_t>(a)] =
                        ks.k_central
                            [static_cast<size_t>(a) * static_cast<size_t>(central_ro.num_samples) +
         ...
         */
        {
            ...
            {
                const double second = projection[j] - 2.0 * projection[j - 1] + projection[j - 2];
        ...
        for (int a = 0; a < 3; ++a)
        {
            ...
            for (int i = first_signal; i < n_adc; ++i)
                extent = std::max(
                    extent,
                    std::fabs(
                        k_echo[static_cast<size_t>(i)][static_cast<size_t>(a)] -
        ...
        for (size_t ax = 0; ax < axes.size(); ++ax)
        {
            ...
            for (int i = first_signal; i < n_adc; ++i)
            {
                ...
                {
                    const double v = k_echo[static_cast<size_t>(i)][static_cast<size_t>(axes[b])];
                    d += v * v;
                }
        ...
        std::vector<std::vector<int>> visits;
        {
            ...
            for (int i = first_signal; i < n_adc; ++i)
            {
                ...
                rep[static_cast<size_t>(i)] =
                    static_cast<int>(visits[static_cast<size_t>(it->second)].size());
                ...
                for (size_t d = 0; d < dims.size(); ++d)
                {
                    if (dims[d].size > 0 && dims[d].size != sizes[d])
                        throw std::runtime_error(
                            ...
                            ", but the acquisition order says it is " + std::to_string(sizes[d]) +
                            ".");
                }
            }
        ...
        result.aux.center_sample = (rev[static_cast<size_t>(central)] != 0)
                ...
                if (amplitude > 0.0 && bandwidth > 0.0)
                {
                    ...
                    if (ascending_slices.size() > 1)
                    {
                        ...
                        for (size_t s = 2; s < ascending_slices.size(); ++s)
                            closest =
                                std::min(closest, ascending_slices[s] - ascending_slices[s - 1]);
    ...
    void apply_labels(Sequence& seq, const AutoLabels& labels, const AutoLabelAux& aux)
    {
        ...
        if (!present.empty())
        {
            ...
            for (size_t i = 0; i < labels.adc_block.size(); ++i)
            {
                ...
                for (size_t f = present.size(); f-- > 0;)
                {
                    ...
                    std::map<std::pair<int32_t, int32_t>, int>::iterator it = label_rows.find(key);
                    if (it == label_rows.end())
                        it = label_rows.emplace(key, seq.register_label_set(key.first, key.second))
                                 .first;
                    head = static_cast<int32_t>(
        ...
        if (aux.has_gridding)
        {
            seq.set_definition(
                ...
                Definition(std::vector<double>(
                    aux.trapezoid_gridding.begin(),
