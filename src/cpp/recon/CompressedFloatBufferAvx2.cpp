...
static inline __m256i scale_and_cast_ps(
    const __m256 _float_val,
    const __m256 _scale,
    const __m256 _sign_mask,
    __m256i& _sign)
{
    const __m256 _half = _mm256_set1_ps(0.5f);

    _sign = _mm256_srai_epi32(_mm256_castps_si256(_mm256_andnot_ps(_sign_mask, _float_val)), 31);

    __m256i _int_val = _mm256_cvtps_epi32(_mm256_round_ps(
        _mm256_fmadd_ps(_mm256_and_ps(_float_val, _sign_mask), _scale, _half),
        _MM_FROUND_TO_ZERO));
...
static inline __m256i compact_epi32(
    const __m256i _int_val,
    const __m256i _sign,
    const __m256i _bitmask)
{
    ...
    __m256i _compact_val = _mm256_and_si256(
        _mm256_blendv_epi8(
            _int_val,
            _mm256_add_epi32(_mm256_xor_si256(_int_val, _bitmask), _one),
            _sign),
        _bitmask);
...
void CompressedFloatBufferAvx2::compress(
    std::vector<float>& d,
    float tolerance,
    uint8_t precision_bits)
{
    if (tolerance <= 0 && (precision_bits < 1 || precision_bits > 31))
    {
        throw std::runtime_error(
            "Argument 'precision_bits' must be between 1 and 31 if 'tolerance' is less than 0.");
    }
    ...
    if (nIterations > 0)
    {
        ...
        {
            _max0 = _mm256_max_ps(_max0, _mm256_and_ps(_mm256_loadu_ps(dptr + i + 0), _sign_mask));

            _max1 = _mm256_max_ps(_max1, _mm256_and_ps(_mm256_loadu_ps(dptr + i + 8), _sign_mask));

            _max2 = _mm256_max_ps(_max2, _mm256_and_ps(_mm256_loadu_ps(dptr + i + 16), _sign_mask));

            _max3 = _mm256_max_ps(_max3, _mm256_and_ps(_mm256_loadu_ps(dptr + i + 24), _sign_mask));
        }
        ...
        __m128 _max = _mm_max_ps(_mm256_castps256_ps128(_max0), _mm256_extractf128_ps(_max0, 1));

        _max = _mm_max_ps(_max, _mm_permute_ps(_max, 0xb1));

        _max = _mm_max_ps(_max, _mm_permute_ps(_max, 0x7e));
    ...
    if (bits_ <= 16)
    {
        ...
        for (size_t n = 0; n < nIterations; i += 32, n++)
        {
            ...
            __m256i _extended_hi0 =
                _mm256_cvtepi32_epi64(_mm256_extracti128_si256(_compact_val0, 1));
            __m256i _extended_lo1 = _mm256_cvtepi32_epi64(_mm256_castsi256_si128(_compact_val1));
            __m256i _extended_hi1 =
                _mm256_cvtepi32_epi64(_mm256_extracti128_si256(_compact_val1, 1));
            __m256i _extended_lo2 = _mm256_cvtepi32_epi64(_mm256_castsi256_si128(_compact_val2));
            __m256i _extended_hi2 =
                _mm256_cvtepi32_epi64(_mm256_extracti128_si256(_compact_val2, 1));
            __m256i _extended_lo3 = _mm256_cvtepi32_epi64(_mm256_castsi256_si128(_compact_val3));
            __m256i _extended_hi3 =
                _mm256_cvtepi32_epi64(_mm256_extracti128_si256(_compact_val3, 1));
            ...
            __m256i _pack_lo = _mm256_or_si256(
                ...
                _mm256_slli_epi64(
                    _mm256_or_si256(
                        _extended_lo1,
                        _mm256_slli_epi64(
                            _mm256_or_si256(_extended_lo2, _mm256_slli_epi64(_extended_lo3, bits_)),
                            bits_)),
                    bits_));
            ...
            __m256i _pack_hi = _mm256_or_si256(
                ...
                _mm256_slli_epi64(
                    _mm256_or_si256(
                        _extended_hi1,
                        _mm256_slli_epi64(
                            _mm256_or_si256(_extended_hi2, _mm256_slli_epi64(_extended_hi3, bits_)),
                            bits_)),
                    bits_));
            ...
            pack(
                ...
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_lo, 0)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_hi, 0)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_lo, 1)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_hi, 1)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_lo, 2)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_hi, 2)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_lo, 3)));
            pack(
                bptr,
                b,
                upshift,
                bits_x_4,
                static_cast<uint64_t>(_mm256_extract_epi64(_pack_hi, 3)));
        }
    }
...
void CompressedFloatBufferAvx2::decompress(float* dptr)
{
    ...
    __m256i _bits_x_idx_0 = _mm256_mullo_epi32(_mm256_set_epi64x(6, 4, 2, 0), _bits);

    __m256i _bits_x_idx_1 = _mm256_mullo_epi32(_mm256_set_epi64x(7, 5, 3, 1), _bits);
    ...
    for (size_t n = 0; n < nIterations; i += 8, n++)
    {
        ...
        __m256i _upshift_0 = _mm256_sub_epi64(_bits_x_idx_0, _mm256_slli_epi64(_sb_0, 3));

        __m256i _upshift_1 = _mm256_sub_epi64(_bits_x_idx_1, _mm256_slli_epi64(_sb_1, 3));

        __m256i _compact_val_0 = _mm256_and_si256(_mm256_srlv_epi64(_b_0, _upshift_0), _bitmask_64);

        __m256i _compact_val_1 = _mm256_and_si256(_mm256_srlv_epi64(_b_1, _upshift_1), _bitmask_64);

        // All values are <32 bits now.
        // Interleave into a single register

        __m256i _compact_val_combined =
            _mm256_or_si256(_compact_val_0, _mm256_slli_si256(_compact_val_1, 4));

        //Convert back to binary
        __m256i _int_val = _mm256_blendv_epi8(
            _mm256_sub_epi32(
                _zero,
                _mm256_add_epi32(_mm256_xor_si256(_compact_val_combined, _bitmask_32), _one)),
            _compact_val_combined,
            _mm256_cmpeq_epi32(_mm256_and_si256(_compact_val_combined, _sign_mask), _zero));

        //Convert to float and scale back
        __m256 _float_val = _mm256_div_ps(_mm256_cvtepi32_ps(_int_val), _scale);
