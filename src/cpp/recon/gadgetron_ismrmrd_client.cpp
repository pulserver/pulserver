...
std::string get_date_time_string()
{
    ...
    str << timeinfo->tm_year + 1900 << "-" << std::setw(2) << std::setfill('0')
        << timeinfo->tm_mon + 1 << "-" << std::setw(2) << std::setfill('0') << timeinfo->tm_mday
        << " " << std::setw(2) << std::setfill('0') << timeinfo->tm_hour << ":" << std::setw(2)
        << std::setfill('0') << timeinfo->tm_min << ":" << std::setw(2) << std::setfill('0')
        << timeinfo->tm_sec;
...
public:
    GadgetronClientDependencyQueryReader(std::string filename)
        : number_of_calls_(0), filename_(filename)
    {
    }
...
public:
    GadgetronClientImageMessageReader(std::string filename, std::string groupname)
        : file_name_(filename), group_name_(groupname)
    {
    }
    ...
    void read_data_attrib(tcp::socket* stream, const ISMRMRD::ImageHeader& h, ISMRMRD::Image<T>& im)
    {
        ...
        if (meta_attrib_length > 0)
        {
            ...
            boost::asio::read(
                *stream,
                boost::asio::buffer(const_cast<char*>(meta_attrib.c_str()), meta_attrib_length));
        ...
        boost::asio::read(*stream, boost::asio::buffer(im.getDataPtr(), im.getDataSize()));
        {
            if (!dataset_)
            {
                {
                    mtx.lock();
                    dataset_ = std::shared_ptr<ISMRMRD::Dataset>(new ISMRMRD::Dataset(
                        file_name_.c_str(),
                        group_name_.c_str(),
...
public:
    ...
    void array2Header(
        ...
        const std::vector<float>& pixelSize,
        HeaderType& header)
    {
        try
        {
            ...
            size_t i;
            for (i = 0; i < 10; i++)
                header.hk.data_type[i] = 0;
            for (i = 0; i < 18; i++)
                header.hk.db_name[i] = 0;
            ...
            // data history
            for (i = 0; i < 80; i++)
                header.hist.descrip[i] = 0;
            for (i = 0; i < 24; i++)
                header.hist.aux_file[i] = 0;
            header.hist.orient = 0;
            for (i = 0; i < 10; i++)
                header.hist.originator[i] = 0;
            for (i = 0; i < 10; i++)
                header.hist.generated[i] = 0;
            for (i = 0; i < 10; i++)
                header.hist.scannum[i] = 0;
            for (i = 0; i < 10; i++)
                header.hist.patient_id[i] = 0;
            for (i = 0; i < 10; i++)
                header.hist.exp_date[i] = 0;
            for (i = 0; i < 10; i++)
                header.hist.exp_time[i] = 0;
            for (i = 0; i < 3; i++)
                header.hist.hist_un0[i] = 0;
            header.hist.views = 0;
...
public:
    ...
    void read_data_attrib(tcp::socket* stream, const ISMRMRD::ImageHeader& h, ISMRMRD::Image<T>& im)
    {
        ...
        std::cout << "Receiving image : " << h.image_series_index << " - " << h.image_index
                  << " - [" << h.matrix_size[0] << " " << h.matrix_size[1] << " "
                  << h.matrix_size[2] << "]" << std::endl;
        ...
        outfileData.write(
            reinterpret_cast<const char*>(im.getDataPtr()),
            sizeof(T) * dim[0] * dim[1] * dim[2] * dim[3]);
        outfileData.close();
    }
...
public:
    GadgetronClientBlobMessageReader(std::string fileprefix, std::string filesuffix)
        : number_of_calls_(0), file_prefix(fileprefix), file_suffix(filesuffix)
    ...
    virtual void read(tcp::socket* socket)
    {
        ...
        boost::asio::read(
            *socket,
            boost::asio::buffer(const_cast<char*>(filenameBuf.c_str()), fileNameLen));
        ...
        if (meta_attrib_length > 0)
        {
            ...
            boost::asio::read(
                *socket,
                boost::asio::buffer(
                    const_cast<char*>(meta_attrib_socket.c_str()),
                    meta_attrib_length));
...
GadgetronClientConnector::GadgetronClientConnector()
    : socket_(0), timeout_ms_(10000), uncompressed_bytes_sent_(0), compressed_bytes_sent_(0),
...
void GadgetronClientConnector::send_gadgetron_info_query(
...
void GadgetronClientConnector::send_ismrmrd_acquisition(ISMRMRD::Acquisition& acq)
{
    ...
    if (trajectory_elements)
    {
        header_bytes_sent_ += boost::asio::write(
            *socket_,
            boost::asio::buffer(&acq.getTrajPtr()[0], sizeof(float) * trajectory_elements));
    }

    if (data_elements)
    {
        uncompressed_bytes_sent_ += boost::asio::write(
            *socket_,
            boost::asio::buffer(&acq.getDataPtr()[0], 2 * sizeof(float) * data_elements));
    }
}

void GadgetronClientConnector::send_ismrmrd_compressed_acquisition_precision(
    ISMRMRD::Acquisition& acq,
    unsigned int compression_precision)
{
    if (!socket_)
    {
        throw GadgetronClientException("Invalid socket.");
    ...
    ISMRMRD::AcquisitionHeader h =
        acq.getHead(); //We will make a copy because we will be setting some flags
    ...
    if (data_elements)
    {
        std::vector<float> input_data(
            (float*)&acq.getDataPtr()[0],
            (float*)&acq.getDataPtr()[0] +
                acq.getHead().active_channels * acq.getHead().number_of_samples * 2);

        std::unique_ptr<CompressedFloatBuffer> comp_buffer(
            CompressedFloatBuffer::createCompressedBuffer());
        ...
        boost::asio::write(
            *socket_,
            boost::asio::buffer(&serialized_buffer[0], serialized_buffer.size()));
    }
}

void GadgetronClientConnector::send_ismrmrd_compressed_acquisition_tolerance(
    ISMRMRD::Acquisition& acq,
    ...
    NoiseStatistics& stat)
{
    ...
    if (data_elements)
    {
        ...
        {
            local_tolerance = local_tolerance * stat.sigma_min * acq.getHead().sample_time_us *
                std::sqrt(stat.noise_dwell_time_us / acq.getHead().sample_time_us);
        }
...
void GadgetronClientConnector::send_ismrmrd_zfp_compressed_acquisition_precision(
    ISMRMRD::Acquisition& acq,
    unsigned int compression_precision)
{
#if defined GADGETRON_COMPRESSION_ZFP
    ...
    if (data_elements)
    {
        ...
        try
        {
            compressed_size = compress_zfp_precision(
                (float*)&acq.getDataPtr()[0],
                acq.getHead().number_of_samples * 2,
                acq.getHead().active_channels,
            ...
            float compression_ratio =
                (1.0 * data_elements * 2 * sizeof(float)) / (float)compressed_size;
...
void GadgetronClientConnector::send_ismrmrd_zfp_compressed_acquisition_tolerance(
    ISMRMRD::Acquisition& acq,
    float compression_tolerance,
    NoiseStatistics& stat)
{
    ...
    if (data_elements)
    {
        ...
        try
        {
            compressed_size = compress_zfp_tolerance(
                (float*)&acq.getDataPtr()[0],
                acq.getHead().number_of_samples * 2,
                acq.getHead().active_channels,
                local_tolerance,
...
void GadgetronClientConnector::send_ismrmrd_waveform(ISMRMRD::Waveform& wav)
{
    ...
    header_bytes_sent_ += boost::asio::write(
        *socket_,
        boost::asio::buffer(&wav.head, sizeof(ISMRMRD::ISMRMRD_WaveformHeader)));
    ...
    if (data_elements)
    {
        header_bytes_sent_ += boost::asio::write(
            *socket_,
            boost::asio::buffer(wav.begin_data(), sizeof(uint32_t) * data_elements));
    }
}

void GadgetronClientConnector::register_reader(
    unsigned short slot,
    std::shared_ptr<GadgetronClientMessageReader> r)
{
    readers_[slot] = r;
}
...
NoiseStatistics get_noise_statistics(
    ...
    unsigned int timeout_ms)
{
    ...
    con.register_reader(
        GADGET_MESSAGE_DEPENDENCY_QUERY,
        std::make_shared<GadgetronClientQueryToStringReader>(result));
...
void send_ismrmrd_acq(
    GadgetronClientConnector& con,
    ISMRMRD::Acquisition& acq_tmp,
    unsigned int compression_precision,
    bool use_zfp_compression,
    float compression_tolerance,
    NoiseStatistics& noise_stats)
{
    try
    {
        if (compression_precision > 0)
        {
            ...
            {
                con.send_ismrmrd_zfp_compressed_acquisition_precision(
                    acq_tmp,
                    compression_precision);
            }
            else
        ...
        else if (compression_tolerance > 0.0)
        {
            if (use_zfp_compression)
            {
                con.send_ismrmrd_zfp_compressed_acquisition_tolerance(
                    acq_tmp,
            ...
            {
                con.send_ismrmrd_compressed_acquisition_tolerance(
                    acq_tmp,
                    compression_tolerance,
                    noise_stats);
            }
...
int Gadgetron::gadgetron_ismrmrd_client(
    ...
    const bool verbose)
{
    ...
    if (open_input_file)
    {
        ismrmrd_dataset = std::shared_ptr<ISMRMRD::Dataset>(
            new ISMRMRD::Dataset(in_filename.c_str(), hdf5_in_group.c_str(), false));
        // Read the header
    ...
    if (!query && !info.empty())
    {
        ...
        if (!noise_id.empty())
        {
            std::cout << "Querying the Gadgetron instance for the dependent measurement: "
                      << noise_id << std::endl;
            noise_stats = get_noise_statistics(
                std::string("GadgetronNoiseCovarianceMatrix_") + noise_id,
                host_name,
                port,
                timeout_ms);
            if (!noise_stats.status)
            {
                std::cout << "WARNING: Dependent noise measurement not found on Gadgetron server. "
                             "Was the noise data processed?"
                          << std::endl;
                if (compression_tolerance > 0.0)
                {
                    std::cout << "  !!!!!! COMPRESSION TOLERANCE LEVEL SPECIFIED, BUT IT IS NOT "
                                 "POSSIBLE TO DETERMINE SIGMA. ASSIMUMING SIGMA == 1 !!!!!!"
                              << std::endl;
                }
            }
            else
            {
                std::cout << "Noise level: Min sigma = " << noise_stats.sigma_min
                          << ", Mean sigma = " << noise_stats.sigma_mean
                          << ", Max sigma = " << noise_stats.sigma_max << std::endl;
            }
        }
    }
    ...
    {
        con.register_reader(
            GADGET_MESSAGE_ISMRMRD_IMAGE,
            std::shared_ptr<GadgetronClientMessageReader>(
                new GadgetronClientAnalyzeImageMessageReader(hdf5_out_group)));
    }
    else
    {
        con.register_reader(
            GADGET_MESSAGE_ISMRMRD_IMAGE,
            std::shared_ptr<GadgetronClientMessageReader>(
                new GadgetronClientImageMessageReader(out_filename, hdf5_out_group)));
    }

    con.register_reader(
        GADGET_MESSAGE_DICOM_WITHNAME,
        std::shared_ptr<GadgetronClientMessageReader>(
            new GadgetronClientBlobMessageReader(std::string(hdf5_out_group), std::string("dcm"))));

    con.register_reader(
        GADGET_MESSAGE_DEPENDENCY_QUERY,
        std::shared_ptr<GadgetronClientMessageReader>(
            new GadgetronClientDependencyQueryReader(std::string(out_filename))));
    con.register_reader(
        GADGET_MESSAGE_TEXT,
        std::shared_ptr<GadgetronClientMessageReader>(new GadgetronClientTextReader()));
    con.register_reader(
        7,
        std::shared_ptr<GadgetronClientResponseReader>(new GadgetronClientResponseReader()));
    ...
    try
    {
            ...
            if (waveforms > 0)
            {
                ...
                while (i < acquisitions && j < waveforms)
                {
                    while (wav_tmp.head.time_stamp < acq_tmp.getHead().acquisition_time_stamp)
                    {
                        ...
                        if (verbose)
                        {
                            std::cout << "--> Send out ismrmrd waveform : " << j << " - "
                                      << wav_tmp.head.scan_counter << " - "
                                      << wav_tmp.head.time_stamp << " - " << wav_tmp.head.channels
                                      << " - " << wav_tmp.head.number_of_samples << " - "
                                      << wav_tmp.head.waveform_id << std::endl;
                        }
                    ...
                    while (acq_tmp.getHead().acquisition_time_stamp <= wav_tmp.head.time_stamp)
                    {
                        ...
                        if (verbose)
                        {
                            std::cout << "==> Send out ismrmrd acq : " << i << " - "
                                      << acq_tmp.getHead().scan_counter << " - "
                                      << acq_tmp.getHead().acquisition_time_stamp << std::endl;
                        }
        ...
        if (verbose)
        {
            ...
            std::cout << "Transmission rate: " << transmitted_mb / transmission_time_s << "MB/s"
                      << std::endl;
        }
...
int main(int argc, char** argv)
{
    ...
    desc.add_options()("help,h", "Produce help message")("query,q", "Dependency query mode")(
        "verbose,v",
        "Verbose mode")("info,Q", po::value<std::string>(), "Query Gadgetron information")(
        "port,p",
        po::value<std::string>(&port)->default_value("9002"),
        "Port")(
        "address,a",
        po::value<std::string>(&host_name)->default_value("localhost"),
        "Address (hostname) of Gadgetron host")(
        "filename,f",
        po::value<std::string>(&in_filename),
        "Input file")(
        "outfile,o",
        po::value<std::string>(&out_filename)->default_value("out.h5"),
        "Output file")(
        "in-group,g",
        po::value<std::string>(&hdf5_in_group)->default_value("/dataset"),
        "Input data group")(
        "out-group,G",
        po::value<std::string>(&hdf5_out_group)->default_value(get_date_time_string()),
        "Output group name")(
        "config,c",
        po::value<std::string>(&config_file)->default_value("default.xml"),
        "Configuration file (remote)")(
        "config-local,C",
        po::value<std::string>(&config_file_local),
        "Configuration file (local)")(
        "loops,l",
        po::value<unsigned int>(&loops)->default_value(1),
        "Loops")(
        "timeout,t",
        po::value<unsigned int>(&timeout_ms)->default_value(10000),
        "Timeout [ms]")(
        "outformat,F",
        po::value<std::string>(&out_fileformat)->default_value("h5"),
        "Out format, h5 for hdf5 and hdr for analyze image")(
        "precision,P",
        po::value<unsigned int>(&compression_precision)->default_value(0),
        "Compression precision (bits)")(
        "tolerance,T",
        po::value<float>(&compression_tolerance)->default_value(0.0),
        "Compression tolerance (fraction of sigma, if no noise stats, assume sigma 1)")
#if defined GADGETRON_COMPRESSION_ZFP
        ("ZFP,Z",
         po::value<bool>(&use_zfp_compression)->default_value(false),
         "Use ZFP library for compression");
#endif //GADGETRON_COMPRESSION_ZFP
    ;
    ...
    if (!vm.count("filename") && !vm.count("query") && !vm.count("info"))
    {
        std::cout << std::endl
                  << std::endl
                  << "\tYou must supply a filename" << std::endl
                  << std::endl;
        std::cout << desc << std::endl;
        return -1;
    }
    ...
    if (vm.count("config-local"))
    {
        ...
        {
            std::cout << "Unable to read local xml configuration: " << config_file_local
                      << std::endl;
            return -1;
        }
    }
    ...
    if (compression_precision > 0 && compression_tolerance > 0.0)
    {
        std::cout << "You cannot supply both compression precision (P) and compression tolerance "
                     "(T) at the same time"
                  << std::endl;
        return -1;
    }
    ...
    return Gadgetron::gadgetron_ismrmrd_client(
        host_name,
        port,
        in_filename,
        out_filename,
