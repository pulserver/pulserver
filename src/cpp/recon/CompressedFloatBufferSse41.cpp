...
static inline __m128i scale_and_cast_ps(
    const __m128 _float_val,
    const __m128 _scale,
    const __m128 _sign_mask,
    __m128i& _sign)
{
    const __m128 _half = _mm_set1_ps(0.5f);

    _sign = _mm_srai_epi32(_mm_castps_si128(_mm_andnot_ps(_sign_mask, _float_val)), 31);

    __m128i _int_val = _mm_cvtps_epi32(_mm_round_ps(
        _mm_add_ps(_mm_mul_ps(_mm_and_ps(_float_val, _sign_mask), _scale), _half),
        _MM_FROUND_TO_ZERO));

    return _int_val;
}

static inline __m128i compact_epi32(
    const __m128i _int_val,
    const __m128i _sign,
    const __m128i _bitmask)
{
    const __m128i _one = _mm_set1_epi32(1);

    __m128i _compact_val = _mm_and_si128(
        _mm_blendv_epi8(_int_val, _mm_add_epi32(_mm_xor_si128(_int_val, _bitmask), _one), _sign),
        _bitmask);

    return _compact_val;
}

void CompressedFloatBufferSse41::compress(
    std::vector<float>& d,
    float tolerance,
    uint8_t precision_bits)
{
    ...
    if (nIterations > 0)
    {
        ...
        {
            _max0 = _mm_max_ps(_max0, _mm_and_ps(_mm_loadu_ps(dptr + i + 0), _sign_mask));

            _max1 = _mm_max_ps(_max1, _mm_and_ps(_mm_loadu_ps(dptr + i + 4), _sign_mask));

            _max2 = _mm_max_ps(_max2, _mm_and_ps(_mm_loadu_ps(dptr + i + 8), _sign_mask));

            _max3 = _mm_max_ps(_max3, _mm_and_ps(_mm_loadu_ps(dptr + i + 12), _sign_mask));
        }
        ...
        _max = _mm_max_ps(_max, _mm_shuffle_ps(_max, _max, 0xb1));

        _max = _mm_max_ps(_max, _mm_shuffle_ps(_max, _max, 0x7e));
            ...
            __m128i _pack_lo = _mm_or_si128(
                ...
                _mm_slli_epi64(
                    _mm_or_si128(
                        _extended_lo1,
                        _mm_slli_epi64(
                            _mm_or_si128(_extended_lo2, _mm_slli_epi64(_extended_lo3, bits_)),
                            bits_)),
                    bits_));
            ...
            __m128i _pack_hi = _mm_or_si128(
                ...
                _mm_slli_epi64(
                    _mm_or_si128(
                        _extended_hi1,
                        _mm_slli_epi64(
                            _mm_or_si128(_extended_hi2, _mm_slli_epi64(_extended_hi3, bits_)),
                            bits_)),
                    bits_));
