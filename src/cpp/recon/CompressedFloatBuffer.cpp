...
void NHLBI::CompressedFloatBuffer::initialize(
    float max_val,
    float tolerance,
    uint8_t precision_bits)
{
    ...
    if (tolerance > 0)
    {
        ...
        if (precision_bits > 31)
        {
            throw std::runtime_error(
                "Tolerance is too small relative to the largest item in the buffer.");
        }
    ...
    size_t bytes_needed =
        static_cast<size_t>(std::ceil((bits_ * elements_) / 8.0f)) + sizeof(uint64_t) -
        sizeof(
            uint8_t); // we write using uint64_t, so ensure there is sufficient space for the last write.
