...
{
public:
    VDSpiral_TrajFunc(f64 kRhoPhi0, f64 kRhoPhi1, f64 phi0 = 0e0) : TrajFunc(0, 0)
    {
        m_kRhoPhi0 = kRhoPhi0;
        m_kRhoPhi1 = kRhoPhi1;
        m_phi0 = phi0;
    ...
    virtual bool getK(v3* k, f64 p)
    {
        ...
        if (m_kRhoPhi0 != m_kRhoPhi1)
        {
            rho = m_kRhoPhi0 * (std::exp(2e0 * (m_kRhoPhi1 - m_kRhoPhi0) * phi) - 1e0) /
                (2e0 * (m_kRhoPhi1 - m_kRhoPhi0));
        }
        else
        {
            rho = m_kRhoPhi0 * phi;
...
public:
          ...
          MrTraj(objGeoPara, objGradPara, 0, 0)
    {
        ...
        vv3 vv3GRO;
        calGrad(NULL, &vv3GRO, NULL, tf, m_objGradPara, 4);
