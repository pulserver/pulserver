...
public:
    ...
    MrTraj(
        const GeoPara& m_objGeoPara,
        const GradPara& m_objGradPara,
        const i64& m_nAcq,
        const i64& m_nSampMax)
        : m_gamma(42.5756e6), m_objGeoPara(m_objGeoPara), m_objGradPara(m_objGradPara),
          m_nAcq(m_nAcq), m_nSampMax(m_nSampMax)
    {
        m_mag = Mag(gMrTraj_nGradSampRsv, gMrTraj_nTrajSampRsv);
        ...
        *pv3M0SP = v3M0PE;
        ...
        for (i64 i = 0; i < (i64)vv3GRO.size() - 1; ++i)
        {
            *pv3M0SP += (*iterGRO + *std::next(iterGRO)) * 1e0 / 2e0; // assume `dt` to be 1
            if (v3::norm(*iterGRO) != 0e0)
                iterGRO_NonZero = iterGRO;
            ++iterGRO;
        }
...
protected:
    ...
    */
    ...
    bool calGRO_MTG(vv3* pvv3G, vf64* pvf64P, const vf64& vf64C, const GradPara& objGradPara)
    {
        ...
        const f64& dt = objGradPara.dt;
        if (pvf64P)
            pvf64P->clear(); // does not supported

        // Prepare arg. for Lustig's function
        f64 g0 = gMrTraj_g0Norm, gfin = gMrTraj_g1Norm, gmax = gLim, smax = sLim, T = dt,
            ds = -1; // ds = 35e-4 for const-Nstep comparison
    ...
    bool intpGrad(vv3* pvv3GRO, vf64* pvf64P, f64 sLim, f64 dt)
    {
        ...
        if (gMrTraj_g0Norm == 0e0 && pvv3GRO)
        {
            ...
            if (pvf64P &&
                !pvf64P->empty()) // null: user does not need p seq, empty: p seq not supported
            {
