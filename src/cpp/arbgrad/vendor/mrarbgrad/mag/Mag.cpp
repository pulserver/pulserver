...
bool Mag::init(TrajFunc* ptTraj, f64 sLim, f64 gLim, f64 dt, i64 oversamp, f64 g0Norm, f64 g1Norm)
{
    m_sLim = sLim;
    m_gLim = gLim;
    m_dt = dt;
...
bool Mag::sovQDE(f64* psol0, f64* psol1, f64 a, f64 b, f64 c)
{
    ...
    if (psol0)
        *psol0 = (-b - (delta < 0 ? 0 : std::sqrt(delta))) / (2 * a);
    if (psol1)
        *psol1 = (-b + (delta < 0 ? 0 : std::sqrt(delta))) / (2 * a);
    return delta >= 0;
}
...
{
    v3 dkdp;
    m_ptfTraj->getDkDp(&dkdp, p);
    v3 d2kdp2;
    m_ptfTraj->getD2kDp2(&d2kdp2, p);
...
f64 Mag::getDp(const v3& v3GPrev, const v3& v3GThis, f64 dt, f64 pPrev, f64 pThis, f64 signDp)
{
    ...
    {
        v3 dkdp;
        m_ptfTraj->getDkDp(&dkdp, pThis + k1 * l);
        f64 dldp = v3::norm(dkdp) * signDp;
        k2 = 1e0 / dldp;
    }
...
f64 Mag::getDp(const v3& v3GPrev, const v3& v3GThis, f64 dt, f64 pPrev, f64 pThis, f64 signDp)
{
    ...
    f64 l = v3::norm(v3GThis) * dt;
    v3 dkdp0;
    m_ptfTraj->getDkDp(&dkdp0, pThis);
    v3 dkdp1;
    m_ptfTraj->getDkDp(&dkdp1, pThis * 2e0 - pPrev);
    ...
    f64 dt)
{
    ...
    if (gNormMin)
        *gNormMin = fabs(*gNormMin);
    if (gNormMax)
        *gNormMax = fabs(*gNormMax);
...
bool Mag::solve(vv3* pvv3G, vf64* pvf64P)
{
    ...
    // backward
    v3 g1Unit;
    ret &= m_ptfTraj->getDkDp(&g1Unit, p1);
    ...
    m_vf64P_Bac.clear();
    m_vf64P_Bac.push_back(p1);
    m_vv3G_Bac.clear();
    m_vv3G_Bac.push_back(g1);
    m_vf64GNorm_Bac.clear();
    m_vf64GNorm_Bac.push_back(v3::norm(g1));
    while (!gMag_enSFS)
    {
    ...
    if (!gMag_enSFS)
        m_intp.fit(m_vf64P_Bac, m_vf64GNorm_Bac);

    nIter += m_vf64P_Bac.size();

    // forward
    v3 g0Unit;
    ret &= m_ptfTraj->getDkDp(&g0Unit, p0);
    ...
    m_vf64P_For.clear();
    m_vf64P_For.push_back(p0);
    m_vv3G_For.clear();
    m_vv3G_For.push_back(g0);
    while (1)
    {
        ...
        // stop or append
        if (std::fabs(m_vf64P_For.back() - p0) >=
            (1 - 1e-6) * std::fabs(p1 - p0)) // || dGNorm <= 0)
        {
    ...
    // deoversamp the para. vec.
    {
        ...
        for (i64 i = 0; i < n; ++i)
        {
            if (i % m_oversamp == m_oversamp / 2)
                pvf64P->push_back(m_vf64P_For[i]);
        }
    }
