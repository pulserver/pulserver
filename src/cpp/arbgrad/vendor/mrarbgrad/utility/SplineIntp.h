...
public:
    ...
    virtual bool fit(const vf64& vf64X, const vf64& vf64Y)
    {
        ...
        for (i64 i = 1; i < nSamp - 1; ++i)
            m_vf64Alpha[i] = (3e0 / m_vf64H[i]) * (m_vf64Y[i + 1] - m_vf64Y[i]) -
                (3e0 / m_vf64H[i - 1]) * (m_vf64Y[i] - m_vf64Y[i - 1]);
        ...
        for (i64 i = nSamp - 2; i >= 0; --i)
        {
            ...
            m_vf64B[i] = (m_vf64A[i + 1] - m_vf64A[i]) / m_vf64H[i] -
                m_vf64H[i] * (m_vf64C[i + 1] + 2e0 * m_vf64C[i]) / 3e0;
    ...
    virtual f64 eval(f64 x, i64 ord = 0)
        const // order: order of derivation, default is 0 (function value)
    {
        i64 idx = getIdx(x);
        ...
        if (ord == 0)
            return (
                m_vf64A[idx] + m_vf64B[idx] * dx + m_vf64C[idx] * dx * dx +
                m_vf64D[idx] * dx * dx * dx);
        if (ord == 1)
            return (m_vf64B[idx] + m_vf64C[idx] * 2e0 * dx + m_vf64D[idx] * 3e0 * dx * dx);
        if (ord == 2)
            return (m_vf64C[idx] * 2e0 + m_vf64D[idx] * 6e0 * dx);
        if (ord == 3)
            return (m_vf64D[idx] * 6e0);
        return 0e0;
    }
