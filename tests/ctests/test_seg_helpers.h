...
#define SEG_TR_WAVEFORM_SET_INIT \
    { \
        0, \
        { \
            { \
...
static TSEG_MAYBE_UNUSED int parse_tr_waveform_set(const char *path, seg_tr_waveform_set *out)
{
    ...
    for (i = 0; i < m; ++i)
    {
        ...
        if (!ws.waveforms[i].time_us || !ws.waveforms[i].gx || !ws.waveforms[i].gy ||
            !ws.waveforms[i].gz)
        {
            free_tr_waveform_set(&ws);
            fclose(f);
...
static TSEG_MAYBE_UNUSED int parse_block_meta(const char *path, block_meta *out)
{
    ...
    while (fscanf(f, "%79s %79s", key, val_str) == 2)
    {
        ...
        /* RF (block 0) */
        else if (strcmp(key, "block_0_rf_delay_us") == 0)
            m.rf_delay_us = atoi(val_str);
        else if (strcmp(key, "block_0_rf_num_samples") == 0)
            m.rf_num_samples = atoi(val_str);
        else if (strcmp(key, "block_0_rf_is_complex") == 0)
            m.rf_is_complex = atoi(val_str);
        else if (strcmp(key, "block_0_rf_num_channels") == 0)
            m.rf_num_channels = atoi(val_str);
        /* Trap gradients: block_B_gA_suffix */
        else if (sscanf(key, "block_%d_g%c_%39s", &idx, &axis_ch, suffix) == 3 && idx < MAX_BLOCKS)
        {
            ...
            else if (strcmp(suffix, "rise_us") == 0)
                m.trap_rise_us[idx][ax] = atoi(val_str);
            else if (strcmp(suffix, "flat_us") == 0)
                m.trap_flat_us[idx][ax] = atoi(val_str);
            else if (strcmp(suffix, "fall_us") == 0)
                m.trap_fall_us[idx][ax] = atoi(val_str);
            else if (strcmp(suffix, "delay_us") == 0)
                m.trap_delay_us[idx][ax] = atoi(val_str);
            /* Arb keys */
            else if (strcmp(suffix, "is_arb") == 0)
                m.has_arb[idx][ax] = atoi(val_str);
            else if (strcmp(suffix, "num_samples") == 0)
                m.arb_num_samples[idx][ax] = atoi(val_str);
        }
        /* Arb keys without axis in gradient pattern — use block_B_gA_KEY */
        /* ADC */
        else if (strcmp(key, "block_2_adc_delay_us") == 0)
            m.adc_delay_us = atoi(val_str);
        /* Segment gap */
        else if (strcmp(key, "rf_adc_gap_us") == 0)
            m.rf_adc_gap_us = atoi(val_str);
    }
...
#define RDF(dst) \
    { \
        float _v; \
        if (fread(&_v, sizeof(float), 1, f) != 1) \
        { \
            fclose(f); \
            return 0; \
        } \
...
#define RDFN(arr, n) \
    ...
    if (out->num_segments < 0 || out->num_segments > MAX_SEGMENTS)
    {
        fprintf(
            stderr,
            "[parse_seg_def] %s: num_segments=%d exceeds MAX_SEGMENTS=%d\n",
            path,
            out->num_segments,
            MAX_SEGMENTS);
        fclose(f);
    ...
    for (s = 0; s < out->num_segments; ++s)
    {
        ...
        if (out->num_blocks[s] < 0 || out->num_blocks[s] > SEG_DEF_MAX_BLOCKS)
        {
            fprintf(
                stderr,
                "[parse_seg_def] %s: segment %d num_blocks=%d exceeds SEG_DEF_MAX_BLOCKS=%d\n",
                path,
                s,
                out->num_blocks[s],
                SEG_DEF_MAX_BLOCKS);
        ...
        for (b = 0; b < out->num_blocks[s]; ++b)
        {
            ...
            if (n > SEG_DEF_MAX_WAVE)
            {
                fprintf(
                    stderr,
                    "[parse_seg_def] %s: segment %d block %d rf_n=%d exceeds SEG_DEF_MAX_WAVE=%d\n",
                    path,
                    s,
                    b,
                    n,
            ...
            for (ax = 0; ax < 3; ++ax)
            {
                ...
                if (n > SEG_DEF_MAX_WAVE)
                {
                    fprintf(
                        stderr,
                        "[parse_seg_def] %s: segment %d block %d axis %d grad_n=%d exceeds "
                        "SEG_DEF_MAX_WAVE=%d\n",
                        path,
                        s,
                        b,
...
static TSEG_MAYBE_UNUSED int parse_fmod_defs(const char *path, fmod_def_file *out)
{
    ...
    if (fread(&out->num_defs, sizeof(int), 1, f) != 1 || out->num_defs < 0 ||
        out->num_defs > MAX_FMOD_DEFS)
    {
        fprintf(
            stderr,
            "[parse_fmod_defs] num_defs=%d (max=%d) file=%s\n",
            out->num_defs,
            MAX_FMOD_DEFS,
            path);
        fclose(f);
    ...
    for (d = 0; d < out->num_defs; ++d)
    {
        ...
        if (fread(&fd->type, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&fd->num_samples, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        ...
        if (ns <= 0 || ns > MAX_FMOD_SAMPLES)
        {
            fprintf(
                stderr,
                "[parse_fmod_defs] def[%d] ns=%d (max=%d) file=%s\n",
                d,
                ns,
                MAX_FMOD_SAMPLES,
                path);
        ...
        if (fread(&fd->raster_us, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&fd->duration_us, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&fd->ref_time_us, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(fd->ref_integral, sizeof(float), 3, f) != 3)
        {
            fclose(f);
            return 0;
        }
        if (fread(fd->waveform_gx, sizeof(float), (size_t)ns, f) != (size_t)ns)
        {
            fclose(f);
            return 0;
        }
        if (fread(fd->waveform_gy, sizeof(float), (size_t)ns, f) != (size_t)ns)
        {
            fclose(f);
            return 0;
        }
        if (fread(fd->waveform_gz, sizeof(float), (size_t)ns, f) != (size_t)ns)
        {
            fclose(f);
            return 0;
        }
...
typedef struct exec_stream_entry
{
    int tr_start_flag;
    int segment_id;
...
typedef struct exec_stream_file
{
    int num_entries;
    exec_stream_entry entries[MAX_SCAN_TABLE_ENTRIES];
} exec_stream_file;

#define SCAN_TABLE_FILE_INIT \
    { \
        0, \
        { \
            { \
...
static TSEG_MAYBE_UNUSED int parse_exec_stream(const char *path, exec_stream_file *out)
{
    FILE *f;
    int i;
    memset(out, 0, sizeof(*out));
    ...
    if (fread(&out->num_entries, sizeof(int), 1, f) != 1 || out->num_entries < 0 ||
        out->num_entries > MAX_SCAN_TABLE_ENTRIES)
    {
        fclose(f);
        return 0;
    }
    ...
    {
        exec_stream_entry *e = &out->entries[i];
        if (fread(&e->tr_start_flag, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->segment_id, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->within_segment_idx, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->rf_amp_hz, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->rf_phase_rad, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->rf_freq_hz, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->gx_amp_hz_per_m, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->gy_amp_hz_per_m, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->gz_amp_hz_per_m, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->adc_flag, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->adc_phase_rad, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->adc_freq_hz, sizeof(float), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->digitalout_flag, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->trigger_flag, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(e->rotmat, sizeof(float), 9, f) != 9)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->freq_mod_id, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
...
static TSEG_MAYBE_UNUSED int parse_fmod_plan(const char *path, fmod_plan_file *out)
{
    ...
    if (fread(&out->num_probes, sizeof(int), 1, f) != 1 || out->num_probes < 0 ||
        out->num_probes > MAX_FMOD_PLAN_PROBES)
    {
        fclose(f);
        return 0;
    }
    ...
    if (fread(&out->num_plans, sizeof(int), 1, f) != 1 || out->num_plans < 0 ||
        out->num_plans > MAX_FMOD_PLAN_ENTRIES)
    {
        fclose(f);
        return 0;
    }
    if (fread(&out->max_samples, sizeof(int), 1, f) != 1 || out->max_samples < 0 ||
        out->max_samples > MAX_FMOD_SAMPLES)
    {
        fclose(f);
        return 0;
    }
    ...
    for (i = 0; i < out->num_plans; ++i)
    {
        ...
        if (fread(&e->def_id, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (fread(&e->num_samples, sizeof(int), 1, f) != 1)
        {
            fclose(f);
            return 0;
        }
        if (e->num_samples < 0 || e->num_samples > out->max_samples)
        {
            fclose(f);
            return 0;
        }
    ...
    if (fread(&out->scan_len, sizeof(int), 1, f) != 1 || out->scan_len < 0 ||
        out->scan_len > MAX_SCAN_TABLE_ENTRIES)
    {
        fclose(f);
        return 0;
    }
