...
def fetch_announcements(owner, repo, category, count, token):
    ...
    filtered = [
        d for d in discussions if d["category"]["name"].lower() == category.lower()
    ]
...
def render_html(announcements):
    ...
    for a in announcements:
        lines.append('<div class="announcement-entry" ' 'style="margin-bottom:1.5em;">')
        lines.append(f'  <h3><a href="{a["url"]}">{a["title"]}</a></h3>')
        lines.append(f'  <p style="color:#666; font-size:0.9em;">' f'{a["date"]}</p>')
        lines.append(f'  <div class="announcement-body">{a["body_html"]}' f"</div>")
        lines.append("</div>")
        lines.append("<hr/>")
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Number of announcements (default: " f"{DEFAULT_COUNT})",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="announcements.html",
