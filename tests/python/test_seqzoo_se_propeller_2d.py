...
def design(**kwargs):
    ...
    return se_propeller_2d.main(n_x=N_X, **kwargs)
...
def test_each_blade_is_one_spin_echo_train():
    ...
    assert sorted(set(labels["ECO"].tolist())) == list(range(WIDTH))
    assert len(labels["ECO"]) == BLADES * WIDTH
...
def test_the_blades_centre_line_meets_the_spin_echo():
    ...
    n_samples = k_adc.shape[1] // len(labels["ECO"])

    row = next(
        index
        for index, (blade, line) in enumerate(
            zip(labels["SEG"].tolist(), labels["ECO"].tolist())
        )
...
def test_the_default_protocol_is_feasible():
    system = pp.Opts()
    report = se_propeller_2d.PLUGIN.validate_protocol(
        system, se_propeller_2d.PLUGIN.get_default_protocol(system)
    )
