...
def _load_plugin_module(plugin_path: Path):
    spec = importlib.util.spec_from_file_location("bridge_gre_radial_3d_plugin", plugin_path)
...
def plugin_module():
    ...
    plugin_path = pulserver_interpreter_root / "package" / "pulserver" / "sequences" / "src" / "gre_radial_3d.py"
    if not plugin_path.is_file():
        pytest.skip(f"gre_radial_3d plugin not found: {plugin_path}")
...
def test_default_protocol_has_expected_keys(plugin_module):
    ...
    for key in [
        ...
        "nx", "nslices", "num_shots", "Rz", "bandwidth", "sequence_type", "user0_value",
    ]:
    ...
    assert "Ry" not in protocol  # in-plane encoding is rotation, not Cartesian PE
...
def test_make_sequence_writes_real_pulseq(plugin_module, tmp_path):
    ...
    output_path = tmp_path / "gre_radial_3d_test.seq"
    ...
    # 3 rotated blocks (prephaser, readout, spoiler) per shot per partition.
