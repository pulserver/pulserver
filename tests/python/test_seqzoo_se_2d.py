...
def design(**kwargs):
    ...
    return se_2d.main(n_x=N_X, n_y=N_Y, **kwargs)
...
def test_a_feasible_protocol_reports_its_scan_time():
    system = pp.Opts()
    report = se_2d.PLUGIN.validate_protocol(
        system, se_2d.PLUGIN.get_default_protocol(system)
    )
...
def test_an_impossible_echo_time_is_refused_through_the_protocol():
    system = pp.Opts()
    protocol = se_2d.PLUGIN.get_default_protocol(system)
    params.set_protocol_value(protocol, UIParam.TE, 1.0)
    report = se_2d.PLUGIN.validate_protocol(system, protocol)
