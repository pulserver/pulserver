...
def _load_plugin_module(plugin_path: Path):
    spec = importlib.util.spec_from_file_location("bridge_mprage_3d_plugin", plugin_path)
...
def plugin_module():
    ...
    plugin_path = pulserver_interpreter_root / "package" / "pulserver" / "sequences" / "src" / "mprage_3d.py"
    if not plugin_path.is_file():
        pytest.skip(f"mprage_3d plugin not found: {plugin_path}")
...
def test_make_sequence_segments_pe_by_etl(plugin_module, tmp_path):
    """With ETL < NY, each partition must fire more than one inversion (one
    per segment) — verified by counting inversion RF blocks."""
    opts = pp.Opts()
    protocol = plugin_module.get_default_protocol(opts)
    _set_value(protocol, "nx", 16)
    _set_value(protocol, "ny", 8)
    _set_value(protocol, "nslices", 2)  # partitions
    _set_value(protocol, "etl", 4)  # 2 segments per partition
    _set_value(protocol, "TR", 8.0)
    _set_value(protocol, "user0_value", 100.0)
    _set_value(protocol, "Trecovery", 50.0)

    output_path = tmp_path / "mprage_3d_segmented.seq"
    plugin_module.make_sequence(opts, protocol, str(output_path))
    ...
    assert inversion_count == 4  # 2 partitions x 2 segments
