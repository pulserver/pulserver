...
def design(**kwargs):
    ...
    return gre_3d.main(n_x=N_X, n_y=N_Y, n_z=N_Z, slab_thickness=SLAB, **kwargs)


def kernel(**kwargs):
    kwargs.setdefault("n_acs", 6)
    kwargs.setdefault("n_acs_z", 4)
    return gre_3d.GRE3DKernel(
...
def test_the_default_protocol_round_trips():
    protocol = gre_3d.PLUGIN.get_default_protocol(pp.Opts())
    assert protocol_to_dict(dict_to_protocol(protocol)) == protocol


def test_a_feasible_protocol_reports_its_scan_time():
    system = pp.Opts()
    report = gre_3d.PLUGIN.validate_protocol(
        system, gre_3d.PLUGIN.get_default_protocol(system)
    )
...
def test_an_impossible_echo_time_is_refused_by_name():
    system = pp.Opts()
    protocol = gre_3d.PLUGIN.get_default_protocol(system)
    params.set_protocol_value(protocol, UIParam.TE, 0.5)
    report = gre_3d.PLUGIN.validate_protocol(system, protocol)
