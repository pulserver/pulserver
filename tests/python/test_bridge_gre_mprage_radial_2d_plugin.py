...
def _load_plugin_module(plugin_path: Path):
    spec = importlib.util.spec_from_file_location("bridge_gre_mprage_radial_2d_plugin", plugin_path)
...
def plugin_module():
    ...
    plugin_path = pulserver_interpreter_root / "package" / "pulserver" / "sequences" / "src" / "gre_mprage_radial_2d.py"
    if not plugin_path.is_file():
        pytest.skip(f"gre_mprage_radial_2d plugin not found: {plugin_path}")
...
def test_default_protocol_has_expected_keys(plugin_module):
    ...
    for key in [
        ...
        "nx", "nslices", "num_shots", "bandwidth", "sequence_type", "preparation_type",
...
@pytest.mark.parametrize(
    ...
    [
        # 2 slices x ceil(8 shots / etl=4) = 2 segments/slice = 4 segments total.
        ("inversion", {"i": 4}),
        ("t2_prep", {"p": 8, "r": 4}),
    ],
)
def test_make_sequence_prep_module_rf_counts(plugin_module, tmp_path, prep_type, expected_rf_use_counts):
    ...
    output_path = tmp_path / "mprage_radial_2d_test.seq"
...
def test_make_sequence_with_mt_adds_saturation_pulses(plugin_module, tmp_path):
    ...
    output_path = tmp_path / "mprage_radial_2d_mt_test.seq"
    plugin_module.make_sequence(opts, protocol, str(output_path))
