...
def design_2d(**kwargs):
    ...
    return fse2D_sequence.main(n_x=32, n_y=16, **kwargs)
...
def design_3d(**kwargs):
    ...
    return fse3D_sequence.main(n_x=32, n_y=8, n_z=4, slab_thickness=64e-3, **kwargs)
...
def test_a_te_beyond_the_train_is_clamped_to_its_last_echo():
    kernel = fse2D_sequence.FSE2DKernel(
        pp.Opts(),
        n_x=32,
        n_y=16,
        n_acs=6,
...
def test_the_2d_protocol_reports_the_echo_grid():
    system = pp.Opts()
    protocol = fse2D_sequence.PLUGIN.get_default_protocol(system)
    report = fse2D_sequence.PLUGIN.validate_protocol(system, protocol)
...
def test_the_traps_schedule_hits_its_control_points():
    schedule = fse3D_sequence.traps_flip_schedule(
