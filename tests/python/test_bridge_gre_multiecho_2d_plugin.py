...
def _load_plugin_module(plugin_path: Path):
    spec = importlib.util.spec_from_file_location("bridge_gre_multiecho_2d_plugin", plugin_path)
...
def plugin_module():
    ...
    plugin_path = pulserver_interpreter_root / "package" / "pulserver" / "sequences" / "src" / "gre_multiecho_2d.py"
    if not plugin_path.is_file():
        pytest.skip(f"gre_multiecho_2d plugin not found: {plugin_path}")
...
def test_default_protocol_has_expected_keys(plugin_module):
    ...
    for key in [
        ...
        "user0_value", "user1_value", "user2_value",
    ]:
        assert key in protocol, f"missing key: {key}"

    assert protocol["sequence_type"]["value"] == "gradient_echo"
    assert "imaging_mode" not in protocol  # single-purpose file: no mode toggle
    assert protocol["user0_value"]["unit"] == "ms"
    assert protocol["user1_value"]["options"] == [0.0, 1.0]
...
def test_make_sequence_echo_train(plugin_module, flyback, tmp_path):
    ...
    protocol = plugin_module.get_default_protocol(opts)
    _set_value(protocol, "user1_value", 1.0 if flyback else 0.0)
    _set_value(protocol, "num_echoes", 3)
    ...
    output_path = tmp_path / "gre_multiecho_2d_test.seq"
    ...
    assert seq.definitions.get("NumEchoes") == 3.0
    ...
    in_train_spacings_ms = np.diff(adc_times_ms[:3])
    assert np.allclose(in_train_spacings_ms, 4.0, atol=1e-3)
