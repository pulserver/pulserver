...
The volume counterpart of :mod:`test_seqzoo_se_2d`: the spin-echo timing and
...
def design(**kwargs):
    ...
    return se_3d.main(n_x=N_X, n_y=N_Y, n_z=N_Z, slab_thickness=SLAB, **kwargs)
...
def test_an_impossible_echo_time_is_refused_through_the_protocol():
    system = pp.Opts()
    protocol = se_3d.PLUGIN.get_default_protocol(system)
    params.set_protocol_value(protocol, UIParam.TE, 1.0)
    report = se_3d.PLUGIN.validate_protocol(system, protocol)
