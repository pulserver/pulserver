"""Routines MATLAB Pulseq defines that PyPulseq has no equivalent of.

There is no second Python implementation to diff against, so what is checked
is the property each routine exists to guarantee: an area hit exactly within
the gradient and slew limits, an energy that scales as the square of the flip
