...
from pulserver.app import gre_radial2D_recon
...
def reconstruct(kspace, trajectory, **kwargs):
    plugin = gre_radial2D_recon.GreRadial2DRecon(**kwargs).spawn()
    ...
    image = gre_radial2D_recon.reconstruct_plane(
...
def test_an_undersampled_scan_goes_through_the_solve_and_beats_the_adjoint(
    ...
    solved = gre_radial2D_recon.reconstruct_plane(
        flat, points, (N, N), direct=False, iterations=25
    )
    adjoint = gre_radial2D_recon.reconstruct_plane(flat, points, (N, N), direct=True)
...
def test_the_plugin_reconstructs_a_streamed_measurement(phantom, coil_maps):
    ...
    results = gre_radial2D_recon.PLUGIN(
