"""The 3D Cartesian reconstruction of the recon zoo.

Driven over the acquisitions the runtime would stream, sampled from an
analytic phantom the way any 3D Cartesian sequence of the zoo samples it:
...
PLUGIN = cartesian3D_recon.Cartesian3DRecon(iterations=20)
...
def acquisitions(kspace, pairs, *, n_samples=N, n_calibration=0, measurement_ends=True):
...
def reconstruct(*args, **kwargs):
    ...
    return PLUGIN(*args, **kwargs)[0].data.transpose(0, 2, 1)
...
def test_the_matrix_comes_from_the_header_not_from_the_last_pair(kspace, context):
    ...
    result = PLUGIN(bucket(kspace, pairs, n_calibration=n_calibration), context)[0]
