...
from pulserver.app import gre_multiecho2D_recon
...
def test_each_echo_reconstructs_to_its_own_series(kspace, phantom):
    results = gre_multiecho2D_recon.PLUGIN(
...
def test_the_echo_count_comes_from_the_header():
    assert gre_multiecho2D_recon.echo_count(header()) == N_ECHOES
    bare = header()
    bare.encoding[0].encodingLimits.contrast = None
    assert gre_multiecho2D_recon.echo_count(bare) == 1
