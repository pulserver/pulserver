...
def test_chunk_and_linear_order_agree() -> None:
    assert sampling.linear_order(5, 2) == [[0, 1], [2, 3], [4]]
    assert sampling.chunk_indices([0, 1, 2, 3, 4], 2) == sampling.linear_order(5, 2)


def test_outer_inner_order() -> None:
    assert sampling.outer_inner_order([0, 1], 2) == [
        [(0, 0), (0, 1)],
        [(1, 0), (1, 1)],
    ]
...
@pytest.mark.parametrize(
    "fn",
    [sampling.fse_linear_order, sampling.fse_radial_order, sampling.fse_radial_adaptive_order],
)
...
def test_radial_order_is_center_out() -> None:
    ...
    etl = 7
    shots = sampling.fse_radial_order(coords, etl)
...
def test_radial_adaptive_early_echoes_are_central() -> None:
    ...
    etl = 8
    shots = sampling.fse_radial_adaptive_order(coords, etl, n_sections=4)
    center = coords.mean(axis=0)
...
def test_shuffling_reproducible_and_incoherent() -> None:
    coords = _coords_grid(8)
    etl = 8
    a = sampling.fse_shuffling_order(coords, etl, seed=42)
    b = sampling.fse_shuffling_order(coords, etl, seed=42)
    c = sampling.fse_shuffling_order(coords, etl, seed=7)
    assert a == b
    assert a != c
    assert _all_views_once(a, len(coords))
    # A shuffled train should not be in strict raster order (incoherence).
    linear = sampling.fse_linear_order(coords, etl)
    assert a != linear


def test_empty_inputs() -> None:
    assert sampling.fse_linear_order(np.empty((0, 2)), 4) == []
    assert sampling.fse_radial_order(np.empty((0, 2)), 4) == []
    assert sampling.fse_shuffling_order(np.empty((0, 2)), 4, seed=0) == []


def test_1d_coords_accepted() -> None:
    shots = sampling.fse_linear_order(np.arange(5.0), 2)
...
def test_random_mask_acceleration_and_calib() -> None:
    ...
    again = sampling.random_mask((32, 32), 4.0, calib=(8, 8), seed=0)
    assert np.array_equal(mask, again)


def test_caipirinha_mask_acceleration_and_shift() -> None:
    mask = sampling.caipirinha_mask((12, 12), 2, 2, delta=1)
    ...
    plain = sampling.caipirinha_mask((12, 12), 2, 2, delta=0)
...
def test_mask_rejects_bad_accel() -> None:
    with pytest.raises(ValueError):
        sampling.random_mask((16, 16), 1.0)
    with pytest.raises(ValueError):
        sampling.poisson_disc_mask((16, 16), 0.5)


def test_golden_and_uniform_angles() -> None:
    golden = sampling.golden_angles(8)
    inc_deg = np.rad2deg(np.diff(golden) % (2 * np.pi))
    assert np.allclose(inc_deg, 111.246, atol=1e-3)
    uniform = sampling.uniform_angles(8)
