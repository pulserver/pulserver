...
def _load_plugin_module(plugin_path: Path):
    spec = importlib.util.spec_from_file_location("bridge_gre_noncart_2d_plugin", plugin_path)
...
def plugin_module():
    ...
    plugin_path = pulserver_interpreter_root / "package" / "pulserver" / "sequences" / "src" / "gre_noncart_2d.py"
    if not plugin_path.is_file():
        pytest.skip(f"gre_noncart_2d plugin not found: {plugin_path}")
...
def test_default_protocol_has_expected_keys(plugin_module):
    ...
    for key in [
        ...
        "nx", "nslices", "num_shots", "sequence_type", "user0_value", "user1_value",
    ]:
        assert key in protocol, f"missing key: {key}"

    assert protocol["sequence_type"]["value"] == "gradient_echo"
    assert "bandwidth" not in protocol  # dwell is pinned to the gradient raster
    assert "phase_fov" not in protocol
    assert protocol["user0_value"]["options"] == [0.0, 1.0]  # trajectory toggle
    assert protocol["user1_value"]["options"] == [0.0, 1.0]  # shot order toggle
...
def test_make_sequence_writes_real_pulseq(plugin_module, tmp_path, trajectory):
    ...
    output_path = tmp_path / "gre_noncart_2d_test.seq"
    ...
    assert re.search(r"^ImagingMode 2d\s*$", content, re.MULTILINE)
    assert re.search(r"^NumShots 8\s*$", content, re.MULTILINE)
    assert re.search(r"^NumSlices 2\s*$", content, re.MULTILINE)

    # 1 rotated block (the arb-gradient readout) per shot per slice.
