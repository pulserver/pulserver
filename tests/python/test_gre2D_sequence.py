...
def design(**kwargs):
    ...
    return gre2D_sequence.main(n_x=N_X, n_y=N_Y, **kwargs)
...
def _kernel(**kwargs):
    kwargs.setdefault("te", 5e-3)
    return gre2D_sequence.GREKernel(pp.Opts(), n_x=N_X, n_y=N_Y, **kwargs)
...
def test_the_default_protocol_round_trips():
    protocol = gre2D_sequence.PLUGIN.get_default_protocol(pp.Opts())
...
def test_a_feasible_protocol_reports_its_scan_time():
    opts = pp.Opts()
    protocol = gre2D_sequence.PLUGIN.get_default_protocol(opts)
    result = gre2D_sequence.PLUGIN.validate_protocol(opts, protocol)
    assert result["valid"]
...
def test_the_scanner_gets_the_binary_form_and_a_reader_gets_the_text_one(tmp_path):
    ...
    system = pp.Opts()
    protocol = gre2D_sequence.PLUGIN.get_default_protocol(system)
...
def test_the_plugin_writes_the_same_sequence_the_repl_builds(tmp_path):
    ...
    gre2D_sequence.PLUGIN.make_sequence(opts, protocol, str(written), offline=True)
