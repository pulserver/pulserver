...
namespace pulseq
{
    namespace detail
    {
        ...
        namespace
        {
            ...
            double second_at(const Piecewise& p, const std::vector<double>& c2, double x)
            {
                ...
                const size_t hi = static_cast<size_t>(
                    std::upper_bound(p.t.begin(), p.t.end(), x) - p.t.begin());
        ...
        namespace
        {
            double mean_cumulative(const Piecewise& p, const std::vector<double>& c2, double x,
                                   double dwell)
            {
        ...
        void KTrajPlan::walk_blocks()
        {
            ...
            for (int b = first_block_; b <= last_block_; ++b)
            {
                ...
                if (row[BLOCK_RF] > 0 && row[BLOCK_RF] <= rf_rows)
                {
                    ...
                    {
                        KTrajRf& event = (role == RfRole::Excitation) ? excitations_[
                            static_cast<size_t>(i_exc)] : refocusings_[static_cast<size_t>(i_ref)];
        ...
        void KTrajPlan::samples(int index, double* out_k, bool include_origin) const
        {
            ...
            for (int a = 0; a < 3; ++a)
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    if (w)
                    {
                        v += amplitude * (opts_.sample_window_average
        ...
        std::array<double, 3> KTrajPlan::sample_k(int index, int sample) const
        {
            ...
            for (int a = 0; a < 3; ++a)
            {
                ...
                if (w)
                {
                    ...
                    v += w->amplitude * (opts_.sample_window_average
                                             ? mean_cumulative(w->p, w->c2, at, dwell)
        ...
        void KTrajPlan::derive_centers()
        {
            ...
            const auto group_by_key = [n_ro](const std::vector<RepeatKey>& keys) {
                ...
                const std::vector<int> canon = canonical_ids(
                    ...
                    [&](int a, int b) {
                        return keys[static_cast<size_t>(a - 1)] ==
                               keys[static_cast<size_t>(b - 1)];
                    });
            ...
            for (int i = 0; i < n_ro; ++i)
            {
                    ...
                    for (int j = 0; j < nn; ++j)
                    {
                        ...
                        for (int a = 0; a < 3; ++a)
                        {
                            ...
                            if (!(g.swept & (1u << a)))
                                continue;
                            const double v =
                                buf[a * nn + j] - k_center_[static_cast<size_t>(a)];
                            d += v * v;
                        }
                ...
                if (g.center >= 0)
                {
                    const double t_in_block = adc_offset_[static_cast<size_t>(i)] +
                                              static_cast<double>(g.center) *
                                                  adc_dwell_[static_cast<size_t>(i)];
        ...
        std::array<double, 3> KTrajPlan::block_origin(int block_index) const
        {
            ...
            if (slot < 0 || slot >= num_range_blocks_)
                return {{0.0, 0.0, 0.0}};
            return {{origin_[static_cast<size_t>(slot) * 3],
        ...
        void KTrajPlan::at_times(const double* times, int count, double* out_k) const
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                for (int a = 0; a < 3; ++a)
                    out_k[static_cast<size_t>(a) * static_cast<size_t>(count) +
