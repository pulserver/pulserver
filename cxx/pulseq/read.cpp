 ...
 *
 * The parsing is not done here.  csrc/src/pulseq already reads both formats,
 * and this file compiles a second, double-precision instance of it (see
 * cxx/include/pulseq/raw64.hpp) and converts what it produces.  Maintaining a
    ...
    namespace
    {
        ...
        void copy_extension_type_ids(const raw::pulseq_file& file, Sequence& seq)
        {
            const std::pair<int, const char*> sections[] = {
                {PULSEQ_EXT_TRIGGER, "TRIGGERS"},   {PULSEQ_EXT_ROTATION, "ROTATIONS"},
                {PULSEQ_EXT_LABELSET, "LABELSET"},  {PULSEQ_EXT_LABELINC, "LABELINC"},
                {PULSEQ_EXT_RF_SHIM, "RF_SHIMS"},   {PULSEQ_EXT_DELAY, "DELAYS"},
            };
    ...
    Sequence read_file(const std::string& path)
    {
        ...
        try
        {
            ...
            // the file actually said.
            {
                ...
                double rasters[4] = {seq.rf_raster_time(), seq.grad_raster_time(),
                                     seq.adc_raster_time(), seq.block_duration_raster()};
            ...
            for (int i = 0; i < file.rf_library_size; ++i)
            {
                ...
                double row[RF_WIDTH] = {
                    src[0], src[1], src[2], src[3], src[4] * US,
                    src[5] * US, src[6], src[7], src[8], src[9],
                };
            ...
            for (int i = 0; i < file.grad_library_size; ++i)
            {
                ...
                {
                    const double row[ARB_WIDTH] = {src[1], src[2], src[3],
                                                   src[4], src[5], src[6] * US};
                    seq.register_arbitrary(row);
                }
                else
                {
                    const double row[TRAP_WIDTH] = {src[1], src[2] * US, src[3] * US,
                                                    src[4] * US, src[5] * US};
            ...
            for (int i = 0; i < file.adc_library_size; ++i)
            {
                ...
                const double row[ADC_WIDTH] = {
                    src[0], src[1] * NS, src[2] * US, src[3], src[4], src[5], src[6], src[7],
                };
            ...
            for (int i = 0; i < file.shapes_library_size; ++i)
            {
                ...
                seq.register_shape(shape.num_uncompressed_samples, shape.samples,
                                   shape.num_samples);
            }
            ...
            for (int i = 0; i < file.labelset_library_size; ++i)
            {
                ...
                seq.register_label_set(static_cast<int32_t>(src[0]),
            ...
            for (int i = 0; i < file.labelinc_library_size; ++i)
            {
                ...
                seq.register_label_inc(static_cast<int32_t>(src[0]),
                                       label_across(static_cast<int>(src[1]), seq));
            }
