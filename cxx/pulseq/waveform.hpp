 ...
 * `cxx/pulseq/` that is not installed.  `Piecewise` is an implementation
 * detail two translation units happen to share -- `trajectory.cpp` and
 * `moments.cpp` -- and promoting it to `cxx/include/pulseq/` would make a
...
namespace pulseq
{
    namespace detail
    {
        ...
        inline ArbSamples arb_samples(const Sequence& seq, const double* row)
        {
            ...
            {
                for (int i = 0; i < n; ++i)
                    out.tt.push_back(out.delay +
                                     (static_cast<double>(i) + 0.5) * out.raster);
            }
            if (out.tt.empty())
                return out;
            ...
            out.shape_end = out.uniform ? out.delay + static_cast<double>(n) * out.raster
                                        : out.tt.back();
            out.valid = true;
            return out;
        }
        ...
        inline Piecewise restore_edges(const ArbSamples& s)
        {
            ...
            for (size_t k = 0; k <= n; ++k)
            {
                const double interpolated = (k == 0)   ? s.first
                                            : (k == n) ? s.last
                                                       : 0.5 * (s.w[k - 1] + s.w[k]);
                edges[k] = (std::fabs(restored[k] - interpolated) <= slack) ? interpolated
                                                                           : restored[k];
            }
         ...
         *
         * Mirrors `_gradient_piece` in `python/pulserver/pypulseq/_pulseqpp.py`
        ...
        inline Piecewise unit_waveform(const Sequence& seq, int32_t grad_id,
        ...
        inline Piecewise unit_waveform_analysis(const Sequence& seq, int32_t grad_id,
                                                double* out_amplitude)
        {
            if (out_amplitude)
                *out_amplitude = 0.0;
