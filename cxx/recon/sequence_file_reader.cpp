...
namespace mrdserver
{
    ...
    namespace
    {
        ...
        void fill_label_limits(
            const std::vector<TrajTableEntry>& table, int es_local, EncodingSpace* es)
        {
            bool first = true;
        ...
        SubseqTrajectory build_subseq_trajectory(
            pulseq::Sequence& seq, const pulseq::KSpace& ks, int subseq_idx, int num_averages)
        {
            ...
            for (size_t r = 0; r < ks.readouts.size(); ++r)
            {
                ...
                for (int axis = 0; axis < 3; ++axis)
                {
                    ...
                    for (int s = 0; s < nsamples; ++s)
                        rows[axis][s] =
                            static_cast<float>(base[axis * nsamples + s]);
                }
                ...
                float* amplitudes[3] = {
                    &entry.gx_amplitude, &entry.gy_amplitude, &entry.gz_amplitude};
                int shot_ids[3];
                for (int axis = 0; axis < 3; ++axis)
                {
                    *amplitudes[axis] =
                        grad_ids[axis] ? static_cast<float>(grad_amplitude(seq, grad_ids[axis]))
                                       : 0.0f;
        ...
        RfEnvelope build_rf_envelope(const pulseq::Sequence& seq, int rf_id)
        {
            ...
            {
                t_us[i] = time.size() == n
                              ? static_cast<float>(time[i] * env.raster_us)
        ...
        void fill_rf_stats(RfDef* def, const RfEnvelope& env)
        {
            ...
            if (pulseq_rf_spectrum_run(plan, env.re_uniform.data(), env.im_uniform.data(),
                                       t_us.data(), static_cast<int>(n), env.center_us) > 0)
            {
        ...
        void append_block_signature(
            const pulseq::Sequence& seq, int block_index, std::vector<int64_t>* out)
        {
            const pulseq::Block block = seq.get_block(block_index);
            out->push_back(static_cast<int64_t>(
                std::llround(block.duration / seq.block_duration_raster())));
            ...
            for (const int grad_id : {block.gx, block.gy, block.gz})
            {
                ...
                const double* row =
                    (seq.grad_kind(grad_id) == pulseq::GradKind::Trap
                         ? seq.trap_library()
                         : seq.arb_library())
        ...
        SequenceDescription derive_sequence_description(
            pulseq::Sequence& seq, const pulseq::KSpace& ks, int subseq_idx, int tr_size)
        {
            ...
            for (int id = 1; id <= seq.rf_library().size(); ++id)
            {
                ...
                const std::array<int, 3> key = {static_cast<int>(rf_row[1]),
            ...
            for (const pulseq::Readout& readout : ks.readouts)
            {
                ...
                const double k_norm = std::hypot(
                    readout.k_echo[0], std::hypot(readout.k_echo[1], readout.k_echo[2]));
                for (int axis = 0; axis < 3; ++axis)
            ...
            {
                const int offset = (readout.block_index - 1 - start) % tr_size;
                const double* base = ks.k_adc.data() +
                                     static_cast<size_t>(readout.sample_offset) * 3;
                double norm_sq = 0.0;
                for (int axis = 0; axis < 3; ++axis)
                {
                ...
                if (offsets.size() > 1)
                {
                    ...
                    if (hi > 0.0)
                    {
                        ...
                        for (size_t j = 0; j < offsets.size(); ++j)
                            role_of[offsets[j]] = k_at_echo[j] <= threshold
                                                      ? centre_role
                                                      : ADC_ROLE_NON_CENTER;
                    }
                }
            ...
            for (int i = 0; i < tr_size; ++i)
            {
                ...
                if (block.rf != 0)
                {
                    ...
                    row.params[1] =
                        static_cast<float>(rf_use_code(rf_use_tag(seq, block.rf)));
                ...
                else if (block.adc != 0)
                {
                    ...
                    row.params[1] =
                        static_cast<float>(seq.adc_library().row(block.adc)[4]);
            ...
            for (const SeqEvent& row : sd.events)
            {
                ...
                for (const RfShapeTuple& tuple : sd.rf_shape_tuples)
                    seen = seen || (tuple.rf_def_id == def_id && tuple.rf_shim_id == shim_id &&
                ...
                const float bandwidth =
                    def_id >= 0 && def_id < static_cast<int>(sd.rf_defs.size())
        ...
        float definition_number(
            const SequenceCache& cache, const char* key, float fallback = 0.0f)
        {
    ...
    SequenceCache read_sequence_files(const std::string& lead_path, int num_averages)
    {
        ...
        for (size_t s = 0; s < chain.size(); ++s)
        {
            ...
            if (declared != defs.end() && !declared->second.empty())
                tr_size = static_cast<int>(
                    std::strtod(declared->second.front().c_str(), nullptr));
            ...
            merge_subseq(
                cache, build_subseq_trajectory(seq, ks, static_cast<int>(s), num_averages));
