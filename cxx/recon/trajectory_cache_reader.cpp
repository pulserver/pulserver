    ...
    const pulseg_recon_definitions &d)
{
    ...
    {
        const pulseq_definition &def = d.definitions[i];
...
EncodingSpace convert_encoding_space(const pulseg_encoding_space &es)
{
    ...
    auto conv_ll = [](const pulseq_label_limit &ll) { return LabelLimit{ll.min, ll.max}; };
...
SequenceCache read_sequence_cache(const std::string &cache_path)
{
    ...
    for (int i = 0; i < traj.num_rotations; ++i)
        std::copy(
            traj.rotation_matrices[i],
            traj.rotation_matrices[i] + 9,
...
void add_sequence_resource_paths(ISMRMRD::IsmrmrdHeader &hdr, int tensor_index, int grad_coil_id)
{
...
void enrich_ismrmrd_header(ISMRMRD::IsmrmrdHeader &hdr, const SequenceCache &cache)
{
    ...
    if (!cache.encoding_spaces.empty())
    {
        ...
        auto read_geometry = [&](int subseq_idx, int geometry_tag, float fov_mm[3], float matrix[3])
        {
