...
// pulseq_file's raw arrays are plain malloc'd C arrays freed via
// PULSEG_FREE by pulseq_file_free(); PULSEG_ALLOC/PULSEG_FREE resolve
...
struct Builder
{
    ...
    std::vector<pulseq_rf_shim_entry> rf_shim_rows;
    std::vector<pulseq_shape> shapes; // pass-through (already decompressed)
...
std::map<int, std::vector<IgnoredLabelRef>> rescan_ignored_labels(const std::string &path)
{
    ...
    while (std::getline(f, line))
    {
        ...
        else if (section == "BLOCKS")
        {
            ...
            int idx;
            long vals[7];
            if (iss >> idx >> vals[0] >> vals[1] >> vals[2] >> vals[3] >> vals[4] >> vals[5] >>
                vals[6])
            {
...
// pulseq_file).
// ---------------------------------------------------------------
void populate_definitions(ExternalSequence &seq, pulseq_file *out, Builder &b)
{
    ...
    out->definitions_library =
        (pulseq_definition *)PULSEG_ALLOC(sizeof(pulseq_definition) * keys.size());
    ...
    for (const auto &key : keys)
    {
        ...
        pulseq_definition &def = out->definitions_library[n++];
        ...
        if (key == "GradientRasterTime")
            out->reserved_definitions_library.gradient_raster_time =
                (float)(std::atof(value.c_str()) * 1e6);
        else if (key == "RadiofrequencyRasterTime")
            out->reserved_definitions_library.radiofrequency_raster_time =
                (float)(std::atof(value.c_str()) * 1e6);
        else if (key == "AdcRasterTime")
            out->reserved_definitions_library.adc_raster_time =
                (float)(std::atof(value.c_str()) * 1e6);
        else if (key == "BlockDurationRaster")
            out->reserved_definitions_library.block_duration_raster =
                (float)(std::atof(value.c_str()) * 1e6);
        else if (key == "Name")
        {
            std::strncpy(
                out->reserved_definitions_library.name,
                value.c_str(),
        ...
        {
            std::strncpy(
                out->reserved_definitions_library.next_sequence,
                value.c_str(),
...
int add_grad_axis(Builder &b, SeqBlock &blk, int channel, float grad_raster_us)
{
    ...
    else if (blk.isExtTrapGradient(channel))
    {
        ...
        row[2] = g.first;
        row[3] = g.last;
        row[4] = (float)add_shape(
            b,
            blk.GetExtTrapGradShape(channel).data(),
    ...
    else
    {
        ...
        row[3] = g.last;
        row[4] =
            (float)add_shape(b, blk.GetArbGradShapePtr(channel), blk.GetArbGradNumSamples(channel));
    ...
    const char *source_path)
{
    ...
    *out_ptr = nullptr;
    ...
    // pulseq_file_free()'s contract that the *struct itself*
    // (not just its inner arrays) must be PULSEG_ALLOC'd -- a stack
    // or `new`-allocated instance must never be passed to it.
    pulseq_file *out = (pulseq_file *)PULSEG_ALLOC(sizeof(pulseq_file));
    if (!out)
    {
        if (err_msg)
    ...
    for (int bi = 0; bi < n_blocks; ++bi)
    {
        ...
        if (blk->isRF())
        {
            ...
            b.rf_use_tags.push_back(
                rf.use == 'e'       ? PULSEG_RF_USE_EXCITATION
                    : rf.use == 'r' ? PULSEG_RF_USE_REFOCUSING
        ...
        if (blk->isRotation())
        {
            ...
            append_ext(b, PULSEQ_EXT_ROTATION, (int)b.rotation_rows.size(), ext_head, ext_prev);
        }
        ...
        if (blk->isTrigger())
        {
            ...
            std::array<float, 4> row = {
                (float)trig.triggerType,
                (float)trig.triggerChannel,
                (float)trig.delay,
                (float)trig.duration};
            b.trigger_rows.push_back(row);
            append_ext(b, PULSEQ_EXT_TRIGGER, (int)b.trigger_rows.size(), ext_head, ext_prev);
        }

        if (blk->isSoftDelay())
        {
            SoftDelayEvent &sd = blk->GetSoftDelayEvent();
            int hint_id = pulseq_hint_id_for_name(sd.hint);
            std::array<float, 4> row =
                {(float)sd.numID, (float)sd.offset, sd.factor, (float)hint_id};
            b.soft_delay_rows.push_back(row);
            append_ext(b, PULSEQ_EXT_DELAY, (int)b.soft_delay_rows.size(), ext_head, ext_prev);
        }

        if (blk->hasRfShim())
        {
            RfShimmingEvent &sh = blk->GetRfShim();
            pulseq_rf_shim_entry entry;
            entry.num_channels = sh.nchan;
            std::memset(entry.values, 0, sizeof(entry.values));
            int nch = sh.nchan;
            if (nch > PULSEQ_MAX_RF_SHIM_CHANNELS)
                nch = PULSEQ_MAX_RF_SHIM_CHANNELS;
            for (int i = 0; i < nch; ++i)
            {
            ...
            append_ext(b, PULSEQ_EXT_RF_SHIM, (int)b.rf_shim_rows.size(), ext_head, ext_prev);
        }

        for (LabelEvent &le : blk->GetLabelSetEvents())
        {
            std::string name =
                (le.numVal.first >= 0) ? seq.getCounterIdAsString(le.numVal.first) : "";
            ...
            if (name.empty())
                continue;
            int label_id = pulseq_label_id_for_name(name.c_str());
            if (label_id < 0)
                continue;
            std::array<float, 2> row = {(float)value, (float)label_id};
            b.labelset_rows.push_back(row);
            append_ext(b, PULSEQ_EXT_LABELSET, (int)b.labelset_rows.size(), ext_head, ext_prev);
        }
        for (LabelEvent &le : blk->GetLabelIncEvents())
        {
            ...
            append_ext(b, PULSEQ_EXT_LABELINC, (int)b.labelinc_rows.size(), ext_head, ext_prev);
        }
        ...
        if (ig != ignored_labels.end())
        {
            ...
            {
                int label_id = pulseq_label_id_for_name(r.name.c_str());
                if (label_id < 0)
                    continue;
    ...
    {
        out->rf_shim_library = (pulseq_rf_shim_entry *)PULSEG_ALLOC(
            sizeof(pulseq_rf_shim_entry) * b.rf_shim_rows.size());
        std::memcpy(
            out->rf_shim_library,
            b.rf_shim_rows.data(),
            sizeof(pulseq_rf_shim_entry) * b.rf_shim_rows.size());
    ...
    if (!b.shapes.empty())
    {
        out->shapes_library = (pulseq_shape *)PULSEG_ALLOC(sizeof(pulseq_shape) * b.shapes.size());
        std::memcpy(out->shapes_library, b.shapes.data(), sizeof(pulseq_shape) * b.shapes.size());
    }
