 ...
 * reader below it (csrc/src/pulseq) and this are the two halves of a Pulseq
...
namespace pulseq
{
    ...
    public:
        ...
        int size() const { return width_ ? static_cast<int>(values_.size()) / width_ : 0; }
        bool empty() const { return values_.empty(); }

        /** Row @p id, 1-based.  Unchecked; callers hold ids they were given. */
        const T* row(int id) const { return values_.data() + static_cast<size_t>(id - 1) * width_; }
        T* row(int id) { return values_.data() + static_cast<size_t>(id - 1) * width_; }
        ...
        void reserve(int rows) { values_.reserve(static_cast<size_t>(rows) * width_); }
        ...
        void resize(int rows) { values_.resize(static_cast<size_t>(rows) * width_, T{}); }
    ...
    public:
        ...
        int length(int id) const { return starts_[id] - starts_[id - 1]; }
        const double* row(int id) const { return values_.data() + starts_[id - 1]; }
        double* row(int id) { return values_.data() + starts_[id - 1]; }
    ...
    public:
        ...
        int num_uncompressed(int id) const { return num_uncompressed_[id - 1]; }
        int num_compressed(int id) const { return data_.length(id); }
        const double* samples(int id) const { return data_.row(id); }
        /** False while a shape is still held as the waveform it was given as. */
        bool is_compressed(int id) const { return is_compressed_[id - 1] != 0; }
        ...
        void assign(const int32_t* num_uncompressed, int count, const int32_t* starts,
    ...
    public:
        ...
        int version_minor() const { return version_minor_; }
        int version_revision() const { return version_revision_; }
        void set_version(int major, int minor, int revision);

        /* -- rasters ---------------------------------------------------- */

        double rf_raster_time() const { return rf_raster_; }
        double grad_raster_time() const { return grad_raster_; }
        double adc_raster_time() const { return adc_raster_; }
        ...
        int num_blocks() const { return static_cast<int>(durations_.size()); }
        ...
        int num_gradients() const { return static_cast<int>(grad_slot_.size()); }
        /** The signed slot: +row for a trapezoid, -row for an arbitrary. */
        const int32_t* grad_slots() const { return grad_slot_.data(); }
        ...
        void set_shapes(const int32_t* num_uncompressed, int count, const int32_t* starts,
                        const double* samples);
