...
from pulserver.app.sequence.gre_radial2D_sequence import spoke_angles
...
def main(
    ...
    seq_filename: str = "gre_stack_of_stars_3d.seq",
    *,
    ...
    Radial spokes in-plane, Cartesian partitions along z --
    :class:`design.RadialStackReadout` from a slab excitation. One spoke
    waveform serves every ``(angle, partition)``: the angle is a ``ROTATIONS``
    extension and the partition an amplitude on the encode pair, so the
    gradient library stays the size of one repetition however large the scan.
    The FOV offset goes through ``TransformFOV`` in server mode.
    :mod:`pulserver.app.noncartesian_stack_recon` reconstructs by NUFFT in-plane
    and FFT along the stack.
    ...
    n_spokes : int or None, optional
        Spokes per partition. None is the radial Nyquist count. Default is
        None.
    ...
    partition_angle_offset_deg : float, optional
        Angle added per partition step, in degrees. Zero plays a given spoke
    ...
    use_rotation_ext : bool, optional
        ...
        second costs a waveform per shot -- one per spoke and partition both,
    ...
    spokes over four partitions, so the stack is legible.
    ...
    .. plot::
       ...
       seq = gre_stack_of_stars3D_sequence(
           n_x=256, n_z=4, n_spokes=16, te=None, tr=None, n_dummy=0,
           spoiling_cycles=6.0,
       )
    ...
    **One repetition**, which is one spoke of one partition: the slab
    excitation, the partition encode and the in-plane prewinder, the
    readout through the centre, and the spoiler.
    ...
    **What the scan covers**: the same golden-angle star repeated at every
    partition, coloured by when each spoke was acquired.
    ...
    **Peripheral nerve stimulation**, under the rheobase/chronaxie model the
    ...
    seq.set_definition(key="Name", value="gre_stack_of_stars_3d")
    seq.set_definition(key="TE", value=kernel.echo_time)
    seq.set_definition(key="TR", value=kernel.repetition_time)
    seq.set_definition(key="Trajectory", value="stack_of_stars")
...
def stack_angles(angles, n_z: int, partition_offset_deg: float):
    """The in-plane angle of every shot of a stack, and where to find it.

    With no partition offset an arm is played at the same angle in every
    partition, so the stack holds one angle per arm. An offset turns the arm a
    little further with each partition, staggering the sampling along kz the
    way a CAIPIRINHA shift does, and every ``(arm, partition)`` pair is then a
    shot of its own.

    Parameters
    ----------
    angles : array-like
        One in-plane angle per arm, in radians.
    n_z : int
        Partitions in the stack.
    partition_offset_deg : float
        Angle added per partition step, in degrees.

    Returns
    -------
    shot_angles : numpy.ndarray
        One angle per shot.
    shot_index : callable
        ``(i_arm, partition) -> int``, where that shot sits in ``shot_angles``.
    """
    angles = np.asarray(angles, dtype=float)
    step = np.deg2rad(float(partition_offset_deg))
    if not step:
        return angles, lambda i_arm, _partition: int(i_arm)
    shots = np.asarray(
        [angle + partition * step for angle in angles for partition in range(n_z)]
    )
    return shots, lambda i_arm, partition: int(i_arm) * n_z + int(partition)


def StackShotKernel(
    seq, readout, blocks, rotation, kz: float, *, acquire: bool, first_extra=()
) -> None:
    """Replay one arm's blocks with the shot's orientation and partition.

    The rotation rides every block that drives an in-plane gradient, when there
    is one to give -- an explicit arm is already turned. The partition scales
    the encode pair. A dummy drops the ADC and the counters that go with it.
    """
    extra_first = list(first_extra)
    for block in blocks:
        events = []
        for event in block:
            if not acquire and getattr(event, "type", "") in ("adc", "labelset"):
                continue
            if event is readout.gz_pre:
...
def StackOfStarsKernel(
    ...
    *,
    ...
    system : pypulseq.Opts
        System limits.
    fov, n_x, n_z, slab_thickness, n_spokes, angle_scheme, flip_angle_deg, \
...
partition_angle_offset_deg, use_rotation_ext
    ...
    readout = design.RadialStackReadout(
        ...
        readout_bandwidth_hz=readout_bandwidth_hz,
        spoiling_cycles=spoiling_cycles,
        # LIN carries the shot: a radial plane has no phase-encode line, so
...
class GreStackOfStars3D(SequencePlugin):
    """The 3D stack-of-stars gradient echo behind the scanner protocol contract."""
...
def protocol_kwargs(system: pp.Opts, protocol: dict[str, dict]) -> dict:
    ...
    return main_kwargs(
        ...
        angle_scheme="uniform" if round(params.user_float(prot, 1, 0.0)) else "golden",
        n_dummy=max(0, round(params.user_float(prot, 2, 32.0))),
    )


PLUGIN = GreStackOfStars3D()
...
ARG_MAP = [
    ...
    ("--spokes", UIParam.user_value(0), float, "Spokes per partition (0 = Nyquist)"),
...
if __name__ == "__main__":
    raise SystemExit(
        run_cli(
            ...
            sys.argv[1:],
            arg_map=ARG_MAP,
            description="Generate a 3D stack-of-stars gradient-echo .seq offline.",
            default_output="gre_stack_of_stars_3d.seq",
        )
    )
