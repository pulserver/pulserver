...
def main(
    ...
    *,
    ...
    """Create an RF-spoiled 3D Cartesian gradient-echo sequence.
    ...
    the readout may be a partial echo. :mod:`pulserver.app.cartesian3D_recon` reads all of
    it back.
    ...
    calibrate while the rest of the scan is still arriving; ``n_dummy`` keeps that
    early centre of k-space out of the transient.
    ...
    seq_filename : str, optional
        Output filename for the .seq file. Default is 'gre_3d.seq'.
    ...
    fov_offset : tuple of float, optional
        ...
        phase and slab axes. Which way those axes point in the magnet is the
    ...
    slab_thickness : float, optional
        Excited slab thickness in meters, which is also the field of view
        along z. Default is 128e-3.
    ...
    n_dummy : int, optional
        ...
        the magnetisation to steady state. The autocalibration rectangle leads
        the traversal, so these are what keeps the centre of k-space -- which
        sets the image contrast -- out of the transient. Default is 64.
    ...
    unmistakable against the encodes; and a small phase-encode and partition
    grid, so the whole traversal fits on a page.
    ...
    .. plot::
       ...
       seq = gre3D_sequence(
           n_x=256, n_y=16, n_z=4, te=None, tr=None, n_acs=0, n_acs_z=0,
    ...
    **One repetition**: the slab excitation and its rephaser, the phase and
    partition encodes alongside the readout prewinder, the readout itself,
    then the spoiler and the rewinders that leave the magnetisation ready for
    the next line.
    ...
    **What the scan covers**, as a phase-encode against partition grid,
    coloured by when in the scan each coordinate was acquired.
    ...
    **Peripheral nerve stimulation**, under the rheobase/chronaxie model the
    ...
    def repetition(
        ...
        ``marks`` ride the excitation, for labels whose value has just
        changed. Label state is sticky, so each stays set until it is set
        again, and one repetition carries the change.
        """
        ...
        if acquire:
            seq.add_block(
                readout.gx, readout.adc, *corkscrew(wave_scale), *readout.adc_labels
            )
        else:
    ...
    # magnetisation the first acquired pair sees is the one every later pair
    # sees. The calibration rectangle -- the centre of k-space -- is acquired
    # first and sets the contrast, so this is what the ordering costs.
    #
    # `ONCE` is what keeps the dummies out of the averages: 1 plays on the
    # first pass only, and the first acquired pair clears it back to 0 so the
    # body repeats.
    ...
    for index, (line, partition) in enumerate(pairs):
        ...
        # The calibration rectangle is segment zero and the rest segment one,
        # which is what lets a reconstruction calibrate the moment the
        # rectangle is complete rather than at the end of the scan.
...
def GRE3DKernel(
    ...
    *,
    ...
    Whatever both the scan loop and the feasibility check need: the slab
    excitation, the readout, the ``(ky, kz)`` pairs the sampling asks for in
    acquisition order, and the total scan time they add up to.

    Building the modules *is* the feasibility check. A TE shorter than the
    repetition can achieve makes :class:`design.LineReadout3D` raise
    ``ValueError``, as does any out-of-range matrix, fov or fraction, so the
    same call the scan loop is built from tells
    :meth:`Gre3D.validate_protocol` whether the protocol is feasible and how
    long it takes -- there is no second timing path to drift out of step with
    the sequence.

    The traversal: the autocalibration rectangle -- every sampled pair whose
    line *and* partition sit inside their ACS bands -- comes first, partitions
    outer and lines inner, then the remaining pairs in the same nesting. A
    product of two per-axis calibration-first orders would interleave ACS and
    non-ACS pairs, which is why the rectangle is carved out explicitly.
...
n_dummy, spoiling_cycles
    ...
    types.SimpleNamespace
        ``excitation``, ``readout``, ``fov``, ``pairs`` (``(line, partition)``
        in acquisition order), ``n_calibration`` (how many of them lead the
        traversal), ``n_averages``, ``echo_time``, ``repetition_time``,
...
class Gre3D(SequencePlugin):
    ...
    def get_default_protocol(self, system: pp.Opts) -> dict[str, dict]:
        ...
        The wave controls appear only where the sequence is wave-encoded:
        with ``WAVE`` unset there is no corkscrew for them to describe, and a
        ...
        return protocol_to_dict(
            {
                ...
                UIParam.TR: DropdownFloatParam(
                    ...
                    incr=0.1,
                    unit="ms",
                    options=[TRPreset.MINIMUM, 8.0, 15.0, 30.0, 100.0],
                ),
                ...
                **(
                    {
                        UIParam.user_name(8): Description(text="Wave cycles"),
                        ...
                        UIParam.user_name(9): Description(text="Wave amplitude"),
    ...
    def validate_protocol(self, system: pp.Opts, protocol: dict[str, dict]) -> dict:
        ...
        Designing the repetition through :func:`GRE3DKernel` is the whole
...
def protocol_kwargs(system: pp.Opts, protocol: dict[str, dict]) -> dict:
    ...
    partition count, which is also the z field of view.
    """
    ...
    return main_kwargs(
        ...
        wave=WAVE,
        wave_cycles=max(1, round(params.user_float(prot, 8, float(WAVE_CYCLES)))),
        wave_amplitude=params.user_float(prot, 9, WAVE_AMPLITUDE * 1e3) * 1e-3,
    )
