...
def main(
    ...
    *,
    ...
    def repetition(readout, slices, i_arm: int, acquire: bool, mark=None) -> None:
    ...
    for slices in kernel.passes:
        ...
        for i_arm in range(len(angles)):
            repetition(readout, slices, i_arm, acquire=True, mark=clear_once)
            clear_once = None
...
def ShotKernel(seq, blocks, rotation, *, acquire: bool, first_extra=()) -> None:
    """Replay one arm's blocks with the shot's orientation injected.

    Every block that drives an in-plane gradient gains the rotation, when there
    is one to give -- an explicit arm is already turned. A dummy drops the ADC
    and the counters that go with it. The blocks are replayed as the module
