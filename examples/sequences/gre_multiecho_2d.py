...
def _load_sibling_module(name: str):
    """Load a same-directory helper module by file path.

    Plugins are loaded standalone (via ``importlib.util.spec_from_file_location``,
    both by the bridge dispatcher and by tests), not as part of a package, so
    a relative import (``from . import _gre_common``) would fail. Loading by
    path instead works regardless of how *this* file itself was loaded, as
    long as ``_gre_common.py`` sits alongside it (true for both the canonical
    and deployed copies).
    """
...
def _read_protocol(prot: dict) -> _Config:
    ...
    cfg.acs_lines = gc.acs_lines_from_protocol(prot, cfg.ny_pe, USER_SLOT_ACS)
...
def _build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a 2D multi-echo Cartesian GRE .seq offline.")
    parser.add_argument("-o", "--output", default="gre_multiecho_2d.seq", help="Output .seq file path")
    ...
    parser.add_argument("--acs-lines", type=float)
...
def _cli(argv: list[str]) -> int:
    ...
    if args.acs_lines is not None:
        gc.set_protocol_value(protocol, UIParam.user_value(USER_SLOT_ACS), args.acs_lines)
