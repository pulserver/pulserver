...
class GreNoncart3DPulseqSequence(Sequence):
    ...
    def make_sequence(self, opts: pp.Opts, protocol: dict[str, dict], output_path: str) -> None:
        ...
        readout_module.set_state(lin_idx=0, par_idx=0, rotation=rotations[0])
        # A spoke is its whole partition loop plus the TR delay that closes it;
        # the delay owns no rows, but it falls *inside* a frame, and only a block
        # between complete passes may interrupt one.
