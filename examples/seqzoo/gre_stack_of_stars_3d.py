...
:mod:`pulserver.reczoo.gre_stack_of_stars_3d` reconstructs by NUFFT in-plane
...
def play_stack(seq, readout, rotation, kz: float, *, acquire: bool, labels=(), first_extra=()) -> None:
    ...
    The rotation rides every block that drives an in-plane gradient; the
    partition scales the encode pair; the labels ride the acquisition; a
    dummy drops the ADC.
    """
    ...
    for block in readout.blocks:
        events = []
        for event in block:
            if not acquire and getattr(event, "type", "") == "adc":
                continue
...
def main(
    ...
    *,
    ...
    def repetition(rotation, partition, kz: float, acquire: bool, mark=None) -> None:
    ...
    for rotation in rotations:
        for partition in range(n_z):
            ...
            repetition(rotation, partition, kz, acquire=True, mark=clear_once)
            clear_once = None
