"""Reconstruction for :mod:`pulserver.app.sequence.zte3D_sequence`.
...
from pulserver.app.recon.gre3D_recon import recon_volume
...
class Zte3DRecon(ReconPlugin):
    ...
    def recon(
        ...
        data = np.concatenate([spoke for spoke, _ in self.spokes], axis=-1)
        ...
        if self.mode == "direct":
            ...
            from pulserver.app.recon.gre_radial2D_recon import smooth_sensitivities
        ...
        return ReconResult(
            _asnumpy(image).transpose(0, 2, 1),
            reference=-1,
