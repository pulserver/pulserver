"""Preparation of zoo EPI data for an upstream reconstruction backend.
...
from pulserver.recon.distortion import run_pyhysco
from pulserver.recon.epi import EpiAcquisitionGroups, partition_epi_acquisitions
...
def run_epi_distortion_correction(
    positive_phase_encode: str | Path, negative_phase_encode: str | Path, phase_encode_direction: int
):
    """Delegate reverse-polarity distortion correction to optional PyHySCO."""
    return run_pyhysco(positive_phase_encode, negative_phase_encode, phase_encode_direction)
