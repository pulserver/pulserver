...
class pulserver_BlockGroup:
    def __init__(self):
        self.label = None  # BLOCKID label value (identifies base block invariant params)
...
def pulserver_build_planning_descriptors(sequences):
    ...
    block_groups_all, block_groups_table = pulserver_get_block_groups(sequences, num_blocks_per_tr)
    ...
    for n, seq in enumerate(sequences):
        ...
        for g_idx, g in enumerate(desc.block_groups):
            ...
            if g_end > desc.num_blocks: g_end = desc.num_blocks
        ...
        for s_idx, s in enumerate(desc.segments):
            ...
            if s_end > desc.num_blocks: s_end = desc.num_blocks
        ...
        for g_idx, g in enumerate(desc.block_groups):
            ...
            if not (s.start <= g.start and g_end <= s.start + s.size):
                raise RuntimeError(f"BlockGroup {g_idx} (blocks {g.start}-{g_end-1}) crosses Segment {seg_idx} (blocks {s.start}-{s.start+s.size-1}) boundaries")
