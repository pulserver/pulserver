...
def write_mrd(
    ...
    for i, acq in enumerate(acq_list):
        acq_arr[i]['head'] = np.frombuffer(acq.getHead(), dtype=ismrmrd_hdf5.acquisition_header_dtype)
        if getattr(acq, 'data', None) is not None and acq.data.size:
            acq_arr[i]['data'] = acq.data.view(np.float32).reshape((2 * acq.active_channels * acq.number_of_samples,))
        else:
            acq_arr[i]['data'] = np.asarray([], dtype=np.float32)
        if getattr(acq, 'traj', None) is not None and acq.traj.size:
            acq_arr[i]['traj'] = acq.traj.view(np.float32).reshape((acq.number_of_samples * acq.trajectory_dimensions,))
        else:
    ...
    if n_wav > 0:
        ...
        for i, wav in enumerate(wav_list):
            wav_arr[i]['head'] = np.frombuffer(wav.getHead(), dtype=ismrmrd_hdf5.waveform_header_dtype)
            if getattr(wav, 'data', None) is not None and wav.data.size:
                wav_arr[i]['data'] = wav.data.view(np.uint32).reshape((wav.channels * wav.number_of_samples,))
            else:
    ...
    with h5py.File(filepath, "w") as file:        
        ...
        dset = grp.create_dataset("data", shape=(n_acq,), maxshape=(None,), dtype=acq_dtype)
        dset[:] = acq_arr
        dset.attrs['next'] = n_acq

        # Waveforms
        if wav_arr is not None:
            wset = grp.create_dataset("waveforms", shape=(n_wav,), maxshape=(None,), dtype=wav_dtype)
            wset[:] = wav_arr
